(function e(t,n,r){function s(o,u){if(!n[o]){if(!t[o]){var a=typeof require=="function"&&require;if(!u&&a)return a(o,!0);if(i)return i(o,!0);var f=new Error("Cannot find module '"+o+"'");throw f.code="MODULE_NOT_FOUND",f}var l=n[o]={exports:{}};t[o][0].call(l.exports,function(e){var n=t[o][1][e];return s(n?n:e)},l,l.exports,e,t,n,r)}return n[o].exports}var i=typeof require=="function"&&require;for(var o=0;o<r.length;o++)s(r[o]);return s})({1:[function(require,module,exports){var AppDispatcher,Dispatcher,PayloadSources,extend=function(child,parent){for(var key in parent){if(hasProp.call(parent,key))child[key]=parent[key]}function ctor(){this.constructor=child}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;return child},hasProp={}.hasOwnProperty;Dispatcher=require("./libs/flux/dispatcher/Dispatcher");PayloadSources=require("./constants/AppConstants").PayloadSources;AppDispatcher=function(superClass){extend(AppDispatcher,superClass);function AppDispatcher(){return AppDispatcher.__super__.constructor.apply(this,arguments)}AppDispatcher.prototype.handleViewAction=function(action){var payload;payload={source:PayloadSources.VIEW_ACTION,action:action};return this.dispatch(payload)};AppDispatcher.prototype.handleServerAction=function(action){var payload;payload={source:PayloadSources.SERVER_ACTION,action:action};return this.dispatch(payload)};return AppDispatcher}(Dispatcher);module.exports=new AppDispatcher},{"./constants/AppConstants":17,"./libs/flux/dispatcher/Dispatcher":19}],2:[function(require,module,exports){var ActionTypes,AppDispatcher,TagStore,XHRUtils;AppDispatcher=require("../AppDispatcher");ActionTypes=require("../constants/AppConstants").ActionTypes;XHRUtils=require("../utils/XHRUtils");TagStore=require("../stores/TagStore");module.exports={selectTags:function(tags){return AppDispatcher.handleViewAction({type:ActionTypes.SELECT_TAGS,value:tags})},selectSortCriterion:function(criterion){AppDispatcher.handleViewAction({type:ActionTypes.SELECT_SORT_CRITERION,value:criterion});return localStorage.setItem("sort-criterion",criterion)},toggleFavorite:function(label){var favoriteTags;favoriteTags=TagStore.getFavoriteTags();if(favoriteTags.indexOf(label)===-1){AppDispatcher.handleViewAction({type:ActionTypes.TOGGLE_FAVORITE_TAG,value:label});return XHRUtils.markTagAsFavorite(label,function(){})}else{AppDispatcher.handleViewAction({type:ActionTypes.TOGGLE_FAVORITE_TAG,value:label});return XHRUtils.unmarkTagAsFavorite(label,function(){})}},markCurrentSearchAsFavorite:function(){var currentFavoriteSearch,markAsFavorite,selectedTags;selectedTags=TagStore.getSelected();currentFavoriteSearch=TagStore.getFavoriteSearch();if(JSON.stringify(selectedTags)===JSON.stringify(currentFavoriteSearch)){markAsFavorite=null}else{markAsFavorite=selectedTags}AppDispatcher.handleViewAction({type:ActionTypes.MARK_SEARCH_AS_FAVORITE,value:markAsFavorite});return XHRUtils.markSearchAsFavorite(markAsFavorite)}}},{"../AppDispatcher":1,"../constants/AppConstants":17,"../stores/TagStore":26,"../utils/XHRUtils":29}],3:[function(require,module,exports){var ActionTypes,AppDispatcher,Options,SELF,TagStore,TaskStore,TaskUtil,XHRUtils,async,ref;async=require("async");AppDispatcher=require("../AppDispatcher");ref=require("../constants/AppConstants"),ActionTypes=ref.ActionTypes,Options=ref.Options;TaskStore=require("../stores/TaskStore");TagStore=require("../stores/TagStore");XHRUtils=require("../utils/XHRUtils");TaskUtil=require("../utils/TaskUtil");module.exports=SELF={createTask:function(content,previousTask){var nextIndex,nextTask,order,rawTask,ref1,step,tasks;tasks=TaskStore.getAll();if(previousTask!=null){nextIndex=tasks.indexOf(previousTask)+1}else{nextIndex=0}nextTask=tasks[nextIndex];ref1=TaskUtil.getNewOrder(previousTask,nextTask),order=ref1.order,step=ref1.step;rawTask={cid:TaskStore.getNextCid(),description:content,order:order,tags:TaskUtil.extractTags(content)};AppDispatcher.handleViewAction({type:ActionTypes.CREATE_TASK,value:{nextIndex:nextIndex,rawTask:rawTask}});return XHRUtils.create(rawTask,function(error,task){var changes,cid;changes={id:task.id,creationDate:task.creationDate,completionDate:task.completionDate};cid=rawTask.cid;AppDispatcher.handleViewAction({type:ActionTypes.UPDATE_TASK,value:{cid:cid,changes:changes}});if(step<=Options.MIN_STEP){return SELF.reindexTasks()}})},moveUp:function(task){var changedPiece,changes,cid,order,previous,previousIndex,previousIndexInSubCollection,previousOfPrevious,ref1,selectedTags,step,subCollection,tasks;selectedTags=TagStore.getSelected();tasks=TaskStore.getAll();subCollection=TaskStore.getByTags(selectedTags);previousIndexInSubCollection=subCollection.indexOf(task)-1;previous=subCollection[previousIndexInSubCollection];previousIndex=tasks.indexOf(previous);previousOfPrevious=tasks[previousIndex-1]||null;if(previousIndex>=0){ref1=TaskUtil.getNewOrder(previousOfPrevious,previous),order=ref1.order,step=ref1.step;changedPiece=[task,previous];changes={order:order};cid=task.cid;AppDispatcher.handleViewAction({type:ActionTypes.UPDATE_TASK,value:{cid:cid,changes:changes}});AppDispatcher.handleViewAction({type:ActionTypes.REORDER_TASK,value:{task:task,changedPiece:changedPiece,index:previousIndex,oldIndex:tasks.indexOf(task)}});return XHRUtils.update(task.id,changes,function(err,task){if(step<=Options.MIN_STEP){return SELF.reindexTasks()}})}},moveDown:function(task){var changedPiece,changes,cid,next,nextIndex,nextIndexInSubCollection,nextOfNext,order,ref1,selectedTags,step,subCollection,tasks;selectedTags=TagStore.getSelected();tasks=TaskStore.getAll();subCollection=TaskStore.getByTags(selectedTags);nextIndexInSubCollection=subCollection.indexOf(task)+1;next=subCollection[nextIndexInSubCollection];nextIndex=tasks.indexOf(next);nextOfNext=tasks[nextIndex+1]||null;if(nextIndex!==tasks.length&&nextIndexInSubCollection!==subCollection.length){ref1=TaskUtil.getNewOrder(next,nextOfNext),order=ref1.order,step=ref1.step;changedPiece=[next,task];changes={order:order};cid=task.cid;AppDispatcher.handleViewAction({type:ActionTypes.UPDATE_TASK,value:{cid:cid,changes:changes}});AppDispatcher.handleViewAction({type:ActionTypes.REORDER_TASK,value:{task:task,changedPiece:changedPiece,index:nextIndex,oldIndex:tasks.indexOf(task)}});return XHRUtils.update(task.id,changes,function(err,task){if(step<=Options.MIN_STEP){return SELF.reindexTasks()}})}},editTask:function(task,newContent){var changes,cid;changes={description:newContent,tags:TaskUtil.extractTags(newContent)};cid=task.cid;AppDispatcher.handleViewAction({type:ActionTypes.UPDATE_TASK,value:{cid:cid,changes:changes}});return XHRUtils.update(task.id,changes,function(err,task){})},toggleState:function(task,isDone){var changes,cid,completionDate;if(isDone){completionDate=Date.now()}else{completionDate=null}changes={done:isDone,completionDate:completionDate};cid=task.cid;AppDispatcher.handleViewAction({type:ActionTypes.UPDATE_TASK,value:{cid:cid,changes:changes}});return XHRUtils.update(task.id,changes,function(err,task){})},removeTask:function(task){AppDispatcher.handleViewAction({type:ActionTypes.REMOVE_TASK,value:task});return XHRUtils.remove(task.id,function(err){})},setArchivedMode:function(isArchived){return AppDispatcher.handleViewAction({type:ActionTypes.SET_ARCHIVED_MODE,value:isArchived})},setSearchQuery:function(searchQuery){return AppDispatcher.handleViewAction({type:ActionTypes.SET_SEARCH_QUERY,value:searchQuery})},archiveTasks:function(tasks){return async.eachLimit(tasks,5,function(task,callback){var payload;AppDispatcher.handleViewAction({type:ActionTypes.ARCHIVE_TASK,value:task.cid});payload={isArchived:true,completionDate:Date.now()};return XHRUtils.update(task.id,payload,function(err){return callback(err)})},function(err){})},restoreTask:function(task){var payload;AppDispatcher.handleViewAction({type:ActionTypes.RESTORE_TASK,value:task.cid});payload={isArchived:false,done:false,completionDate:null};return XHRUtils.update(task.id,payload,function(err,task){})},reindexTasks:function(){AppDispatcher.handleViewAction({type:ActionTypes.SET_REINDEX_STATE,value:true});return XHRUtils.reindex(function(err,tasks){location.reload();return AppDispatcher.handleViewAction({type:ActionTypes.SET_REINDEX_STATE,value:false})})}}},{"../AppDispatcher":1,"../constants/AppConstants":17,"../stores/TagStore":26,"../stores/TaskStore":27,"../utils/TaskUtil":28,"../utils/XHRUtils":29,async:30}],4:[function(require,module,exports){var Backbone,Polyglot;Backbone=require("backbone");Polyglot=require("node-polyglot");Backbone.$=require("jquery");module.exports={initialize:function(){var TagStore,TaskStore,favoriteSearch,locales,localesLoader;this.locale=window.locale;delete window.locale;this.polyglot=new Polyglot({locale:this.locale});localesLoader={en:require("./locales/en"),fr:require("./locales/fr")};locales=localesLoader[this.locale];if(locales==null){locales=localesLoadre["en"]}this.polyglot.extend(locales);window.t=this.polyglot.t.bind(this.polyglot);TaskStore=require("./stores/TaskStore");TagStore=require("./stores/TagStore");this.router=require("./router");window.router=this.router;Backbone.history.start();favoriteSearch=TagStore.getFavoriteSearch();this.router.goToDefault(favoriteSearch);if(typeof Object.freeze==="function"){return Object.freeze(this)}}}},{"./locales/en":22,"./locales/fr":23,"./router":25,"./stores/TagStore":26,"./stores/TaskStore":27,backbone:31,jquery:34,"node-polyglot":36}],5:[function(require,module,exports){var React,TagActionCreator,TaskActionCreator,div,i,ref,styler;React=require("react/addons");TaskActionCreator=require("../actions/TaskActionCreator");TagActionCreator=require("../actions/TagActionCreator");ref=React.DOM,div=ref.div,i=ref.i;styler=React.addons.classSet;module.exports=React.createClass({displayName:"ActionsBar",render:function(){var archiveProperties,archiveStyles,favoriteProperties,favoriteStyles,saveProperties,saveStyles,serializedFavoriteSearch,serializedSelectedTags;archiveStyles=styler({"fa fa-archive":true,disable:this.props.tasksDone.length===0});archiveProperties={className:archiveStyles,role:"button",title:t("archive button title"),onClick:this.onArchiveClicked};saveStyles=styler({"fa fa-bookmark disable":true});saveProperties={className:saveStyles,role:"button",title:t("coming soon")};serializedSelectedTags=JSON.stringify(this.props.selectedTags);serializedFavoriteSearch=JSON.stringify(this.props.favoriteSearch);favoriteStyles=styler({"fa fa-star":true,"is-favorite":serializedFavoriteSearch===serializedSelectedTags});favoriteProperties={className:favoriteStyles,role:"button",title:t("favorite button title"),onClick:this.onFavoriteClicked};return div({id:"actions"},t("actions headline"),i(archiveProperties),i(saveProperties),i(favoriteProperties))},onFavoriteClicked:function(){return TagActionCreator.markCurrentSearchAsFavorite()},onArchiveClicked:function(){return TaskActionCreator.archiveTasks(this.props.tasksDone)}})},{"../actions/TagActionCreator":2,"../actions/TaskActionCreator":3,"react/addons":38}],6:[function(require,module,exports){var KeyboardKeys,React,input,ref,span;React=require("react/addons");ref=React.DOM,input=ref.input,span=ref.span;KeyboardKeys=require("../constants/AppConstants").KeyboardKeys;module.exports=React.createClass({displayName:"AdjustableInput",render:function(){var inputAttributes,spanAttributes,wrapperAttributes;inputAttributes={style:{width:this.state.width},ref:"input",type:"text",value:this.state.content,onChange:this.onChange,onBlur:this.onBlur,onKeyUp:this.onKeyUp,className:this.props.className,placeholder:this.props.placeholder};spanAttributes={ref:"size-calculator",className:"size-calculator"};wrapperAttributes={};if(this.props.id!=null){wrapperAttributes.id=this.props.id}return span(wrapperAttributes,input(inputAttributes),span(spanAttributes,this.state.content))},onChange:function(){var node;node=this.refs["input"].getDOMNode();return this.setState({content:node.value})},onKeyUp:function(event){var key;key=event.keyCode||event.charCode;if(key===KeyboardKeys.ENTER){this.props.onSubmitHandler(this.state.content);return this.setState({content:""})}},onBlur:function(){var node;node=this.refs["input"].getDOMNode();if(node.value.length===0){return this.setState({width:150})}},componentDidUpdate:function(){var node,notInitialState,rects,width;node=this.refs["size-calculator"].getDOMNode();rects=node.getClientRects()[0];if(rects!=null){width=node.getClientRects()[0].width;notInitialState=this.state.content.length>0||this.state.width>150;if(this.state.width!==width&&notInitialState){return this.setState({width:width})}}},getInitialState:function(){return{width:150,content:""}}})},{"../constants/AppConstants":17,"react/addons":38}],7:[function(require,module,exports){var Menu,React,StoreWatchMixin,TagStore,TaskList,TaskStore,div,nav,p,ref,styler;React=require("react/addons");ref=React.DOM,div=ref.div,p=ref.p,nav=ref.nav;Menu=React.createFactory(require("./menu"));TaskList=React.createFactory(require("./task-list"));TaskStore=require("../stores/TaskStore");TagStore=require("../stores/TagStore");styler=React.addons.classSet;StoreWatchMixin=require("../mixins/store_watch_mixin");module.exports=React.createClass({displayName:"Application",mixins:[StoreWatchMixin([TagStore,TaskStore])],getStateFromStores:function(){var selectedTags,tasks;selectedTags=TagStore.getSelected();tasks=TaskStore.getByTags(selectedTags);return{tasks:tasks,selectedTags:selectedTags,tagTree:TagStore.getTree(),sortCriterion:TagStore.getSortCriterion(),isArchivedMode:TaskStore.isArchivedMode(),untaggedTasks:TaskStore.getUntagged(),numTasks:TaskStore.getNumTasks(),numArchivedTasks:TaskStore.getNumArchivedTasks(),searchQuery:TaskStore.getSearchQuery(),tasksDone:tasks.filter(function(task){return task.done}),isReindexing:TaskStore.isReindexing(),favoriteSearch:TagStore.getFavoriteSearch()}},render:function(){var appStyles;appStyles=styler({menuOpen:this.state.isMenuOpen});return div({role:"application",className:appStyles},Menu({selectedTags:this.state.selectedTags,tree:this.state.tagTree,sortCriterion:this.state.sortCriterion,isArchivedMode:this.state.isArchivedMode,untaggedTasks:this.state.untaggedTasks,numTasks:this.state.numTasks,numArchivedTasks:this.state.numArchivedTasks,onOpenMenu:this.openMenu}),div({className:"container"},TaskList({selectedTags:this.state.selectedTags,tasks:this.state.tasks,isArchivedMode:this.state.isArchivedMode,searchQuery:this.state.searchQuery,tasksDone:this.state.tasksDone,onOpenMenu:this.openMenu,favoriteSearch:this.state.favoriteSearch})),this.state.isReindexing?(div({id:"block"}),div({id:"modal"},p(null,t("reindexing message")))):void 0)},getInitialState:function(){return{isMenuOpen:false}},openMenu:function(){var isMenuOpen;isMenuOpen=!this.state.isMenuOpen;return this.setState({isMenuOpen:isMenuOpen})}})},{"../mixins/store_watch_mixin":24,"../stores/TagStore":26,"../stores/TaskStore":27,"./menu":11,"./task-list":14,"react/addons":38}],8:[function(require,module,exports){var React,a,div,ref,span,styler;React=require("react/addons");ref=React.DOM,div=ref.div,span=ref.span,a=ref.a;styler=React.addons.classSet;module.exports=React.createClass({displayName:"BreadcrumbItem",render:function(){var classes,isTag,ref1,removeProperties,value,wrapperProperties;isTag=this.props.type==="tag";classes=styler({"breadcrumb-item":true,excluded:(ref1=this.props.tag)!=null?ref1.isExcluded:void 0,"notice-delete-action":this.state.removeHovered});wrapperProperties={className:classes,onClick:this.props.toggleModeHandler,key:this.props.key};if(!isTag){value='"'+this.props.label+'"'}else{value=this.props.tag.label}removeProperties={onMouseOver:this.onMouseOver,onMouseOut:this.onMouseOut,onClick:function(_this){return function(event){event.stopPropagation();return _this.props.removeHandler()}}(this)};return div(wrapperProperties,span(null,value),a(removeProperties,"×"))},onMouseOver:function(){return this.setState({removeHovered:true})},onMouseOut:function(){return this.setState({removeHovered:false})},getInitialState:function(){return{removeHovered:false}}})},{"react/addons":38}],9:[function(require,module,exports){var AdjustableInput,BreadcrumbItem,React,h1,i,input,ref,span;React=require("react/addons");ref=React.DOM,h1=ref.h1,i=ref.i,span=ref.span,input=ref.input;BreadcrumbItem=React.createFactory(require("./breadcrumb-item"));AdjustableInput=React.createFactory(require("./adjustable-input"));module.exports=React.createClass({displayName:"Breadcrumb",render:function(){var title;title=this.getTitle();return h1({id:"breadcrumb"},i({className:"fa fa-bars",onClick:this.props.onOpenMenu}),span(null,title),this.props.selectedTags!=null?this.renderSelectedTags():void 0,this.props.searchQuery!=null?this.renderSearchInput():void 0,this.props.selectedTags==null||this.props.selectedTags!=null&&!this.hasNoTagSelected()?AdjustableInput({id:"add-tag-form",className:"add-tag",placeholder:t("search tag input"),onSubmitHandler:this.onSubmit}):void 0)},renderSelectedTags:function(){return this.props.selectedTags.map(function(_this){return function(tag,index){return BreadcrumbItem({key:index,type:"tag",tag:tag,removeHandler:_this.removeHandler.bind(_this,tag),toggleModeHandler:_this.toggleModeHandler.bind(_this,tag)})}}(this))},renderSearchInput:function(){var translationKey;translationKey="match criterion";if(this.hasNoTagSelected()){translationKey=translationKey+" no tag"}else{translationKey=translationKey+" with tag"}return BreadcrumbItem({key:"search-query",type:"search",label:this.props.searchQuery,removeHandler:this.removeHandler})},removeHandler:function(tag){var index,newTagsList,ref1,searchQuery;newTagsList=(ref1=this.props.selectedTags)!=null?ref1.slice(0):void 0;if(tag!=null&&newTagsList!=null){index=newTagsList.indexOf(tag);newTagsList.splice(index,1)}else{searchQuery=null}return this.buildUrl(newTagsList,searchQuery)},toggleModeHandler:function(tag){var index,newTagsList,ref1;newTagsList=(ref1=this.props.selectedTags)!=null?ref1.slice(0):void 0;if(newTagsList!=null){index=this.props.selectedTags.indexOf(tag);newTagsList[index]={label:tag.label,isExcluded:!tag.isExcluded}}return this.buildUrl(newTagsList,this.props.searchQuery)},onSubmit:function(newTagValue){var isExcluded,newTagsList,ref1,searchQuery;searchQuery=this.props.searchQuery;newTagsList=(ref1=this.props.selectedTags)!=null?ref1.slice(0):void 0;if(newTagValue.indexOf("#")===0||newTagValue.indexOf("!#")===0){isExcluded=newTagValue.indexOf("!")===0;newTagValue=newTagValue.replace(/[!#]*/,"");if(newTagsList!=null){newTagsList.push({label:newTagValue,isExcluded:isExcluded})}else{newTagsList=[{label:newTagValue,isExcluded:isExcluded}]}}else{searchQuery=newTagValue}return this.buildUrl(newTagsList,searchQuery)},buildUrl:function(tagsList,searchQuery){var formattedList,location,prefix,query;if(tagsList!=null){formattedList=tagsList.map(function(tag){if(tag.isExcluded){return"!"+tag.label}else{return""+tag.label}});formattedList=formattedList.join("/")}else{formattedList=""}query="";if((tagsList!=null?tagsList.length:void 0)>0){if(this.props.isArchivedMode){prefix="archivedByTags"}else{prefix="todoByTags/"}if(searchQuery!=null){query="/;search/"+searchQuery}}else if(searchQuery!=null){prefix="search/";query=searchQuery}else{if(this.props.isArchivedMode){prefix="archived"}else{prefix="#"}}location="#"+prefix+formattedList+query;return window.router.navigate(location,true)},hasNoTagSelected:function(){var ref1;return this.props.selectedTags==null||((ref1=this.props.selectedTags)!=null?ref1.length:void 0)===0},getInitialState:function(){return{inputContent:""}},getTitle:function(){var option,title;if(this.props.selectedTags==null){if(this.props.isArchivedMode){title=t("all archived tasks")}else{title=t("all tasks")}}else if(this.hasNoTagSelected()){title=t("untagged tasks")}else{option={smart_count:this.props.selectedTags.length};if(this.props.isArchivedMode){title=t("archived tasks of",option)}else{title=t("tasks of",option)}}return title}})},{"./adjustable-input":6,"./breadcrumb-item":8,"react/addons":38}],10:[function(require,module,exports){var React,a,i,li,ref,span,styler,ul;React=require("react/addons");ref=React.DOM,li=ref.li,a=ref.a,i=ref.i,span=ref.span,ul=ref.ul;styler=React.addons.classSet;module.exports=React.createClass({displayName:"MenuItem",getInitialState:function(){return{favorite:false}},render:function(){var canBeFav,classNames,label,linkClasses,linkStyle;label=this.props.tag.label;canBeFav=this.props.onFavorite!=null&&this.props.depth===0;classNames=styler({"menu-tag":true,active:this.props.isActive,selected:this.props.isSelected,magic:this.props.magic});linkStyle={paddingLeft:(this.props.depth+1)*20};linkClasses=styler({favorite:canBeFav&&this.props.tag.isFavorite});return li({className:classNames},a({href:this.props.url,title:this.getTitle(),style:linkStyle,className:linkClasses},i({className:"fa fa-tag"}),span(null,label+" ("+this.getCount()+")"),canBeFav?i({className:"fa fa-star",onClick:this.onFavorite}):void 0),this.props.getSubmenu(this.props.depth+1))},getCount:function(){var count,doneCount,ref1,todoCount;ref1=this.props.tag,count=ref1.count,doneCount=ref1.doneCount;todoCount=count-doneCount;if(todoCount!==count&&todoCount!==0){return todoCount+" / "+count}else{return count}},getTitle:function(){var count,doneCount,label,ref1,smart_count,todoCount;ref1=this.props.tag,label=ref1.label,count=ref1.count,doneCount=ref1.doneCount;todoCount=count-doneCount;smart_count=doneCount;return t("tag title",{label:label,todoCount:todoCount,smart_count:smart_count})},onFavorite:function(event){event.preventDefault();return this.props.onFavorite()}})},{"react/addons":38}],11:[function(require,module,exports){var MenuItem,React,SortCriterions,TagActionCreator,_,a,classer,div,i,li,nav,ref,span,ul;React=require("react/addons");_=require("underscore");ref=React.DOM,nav=ref.nav,div=ref.div,i=ref.i,ul=ref.ul,li=ref.li,a=ref.a,span=ref.span;MenuItem=React.createFactory(require("./menu-item"));SortCriterions=require("../constants/AppConstants").SortCriterions;TagActionCreator=require("../actions/TagActionCreator");classer=React.addons.classSet;module.exports=React.createClass({displayName:"Menu",render:function(){var archivedMenu,menu,todoMenu;archivedMenu={id:"archived",link:"#archived",label:t("archived"),count:this.props.numArchivedTasks};todoMenu={id:"tobedone",link:"#",label:t("todo"),count:this.props.numTasks};if(this.props.isArchivedMode){menu=[todoMenu,archivedMenu]}else{menu=[archivedMenu,todoMenu]}return nav({id:"menu",role:"navigation"},i({className:"fa fa-arrow-left",onClick:this.props.onOpenMenu}),ul(null,li({id:menu[0].id,className:"first-level"},a({href:menu[0].link},menu[0].label+" ("+menu[0].count+")")),li({id:menu[1].id,className:"first-level active"},a({href:menu[1].link},menu[1].label+" ("+menu[1].count+")"),this.getSortMenu(),this.getSubmenu(0))))},getSortMenu:function(){var alphaProperties,classes,countProperties;classes=classer({"selected-sort":this.props.sortCriterion===SortCriterions.COUNT});countProperties={className:classes,title:"brah",onClick:this.onSelectCriterion.bind(this,SortCriterions.COUNT)};classes=classer({"selected-sort":this.props.sortCriterion===SortCriterions.ALPHA});alphaProperties={className:classes,onClick:this.onSelectCriterion.bind(this,SortCriterions.ALPHA)};return ul({className:"sorts"},li(alphaProperties,a({href:"#",title:t("sort numeric"),className:"fa fa-sort-alpha-asc"}," ")),li(countProperties,a({href:"#",title:t("sort alpha"),className:"fa fa-sort-numeric-desc"}," ")))},getSubmenu:function(depth){var tags;tags=this.props.tree[depth];return ul({className:"submenu"},depth===0&&this.props.untaggedTasks.length>0?this.getUntaggedMenuItem():void 0,tags.map(function(_this){return function(tag){return _this.getMenuItem(tag,depth)}}(this)))},getMenuItem:function(tag,depth){var currentIndex,getSubmenuHandler,isActive,isLeaf,label,prefix,ref1,selectedTagNames,tagsInUrl,url;label=tag.label;selectedTagNames=(ref1=this.props.selectedTags)!=null?ref1.map(function(tag){return tag.label}):void 0;if((selectedTagNames!=null?selectedTagNames[depth]:void 0)===label){getSubmenuHandler=this.getSubmenu}else{getSubmenuHandler=function(){}}currentIndex=selectedTagNames!=null?selectedTagNames.indexOf(label):void 0;isActive=currentIndex===depth;isLeaf=depth+1===(selectedTagNames!=null?selectedTagNames.length:void 0);tagsInUrl=(selectedTagNames!=null?selectedTagNames.slice(0,depth):void 0)||[];if((!_.contains(tagsInUrl,label)||(selectedTagNames!=null?selectedTagNames.length:void 0)>depth+1)&&!(currentIndex+1===(selectedTagNames!=null?selectedTagNames.length:void 0)&&depth===currentIndex)){tagsInUrl.push(label)}if(this.props.isArchivedMode){if(tagsInUrl.length>0){prefix="archivedByTags"}else{prefix="archived"}}else{if(tagsInUrl.length>0){prefix="todoByTags"}else{prefix=""}}url="#"+prefix;if(tagsInUrl.length>0){url="#"+prefix+"/"+tagsInUrl.join("/")}return MenuItem({key:label+"-"+depth,tag:tag,depth:depth,isActive:isActive,isSelected:isActive&&isLeaf,getSubmenu:getSubmenuHandler,onFavorite:this.onFavorite.bind(this,tag),url:url})},onSelectCriterion:function(criterion,event){event.preventDefault();return TagActionCreator.selectSortCriterion(criterion)},onFavorite:function(tag){return TagActionCreator.toggleFavorite(tag.label)},getUntaggedMenuItem:function(){var isActive,ref1,url;isActive=((ref1=this.props.selectedTags)!=null?ref1.length:void 0)===0;if(this.props.isArchivedMode){if(isActive){url="#archived"}else{url="#archivedByTags/"}}else{if(isActive){url="#"}else{url="#todoByTags/"}}return MenuItem({key:"untagged",tag:{label:t("untagged"),count:this.props.untaggedTasks.length,doneCount:this.props.untaggedTasks.filter(function(task){return task.done}).length},depth:0,isActive:isActive,isSelected:isActive,getSubmenu:function(){},url:url,magic:true})}})},{"../actions/TagActionCreator":2,"../constants/AppConstants":17,"./menu-item":10,"react/addons":38,underscore:202}],12:[function(require,module,exports){var React,div,p,ref,styler;React=require("react/addons");ref=React.DOM,div=ref.div,p=ref.p;styler=React.addons.classSet;module.exports=React.createClass({displayName:"ProgressBar",render:function(){var fillerAttributes,progress,styles,wrapperAttributes;progress=this.props.tasksDone.length/this.props.tasks.length*100;progress=Math.round(progress,1);styles=styler({expanded:this.state.expanded});wrapperAttributes={id:"progressbar",className:styles,onMouseOver:this.onMouseOver,onMouseOut:this.onMouseOut};fillerAttributes={style:{width:progress+"%"}};return div(wrapperAttributes,div(fillerAttributes),p(null,progress+"%"))},onMouseOver:function(){return this.setState({expanded:true})},onMouseOut:function(){return this.setState({expanded:false})},getInitialState:function(){return{expanded:false}}})},{"react/addons":38}],13:[function(require,module,exports){var React,_,button,div,i,input,label,ref,styler;React=require("react/addons");_=require("underscore");ref=React.DOM,div=ref.div,input=ref.input,label=ref.label,button=ref.button,i=ref.i;styler=React.addons.classSet;module.exports=React.createClass({displayName:"TaskButton",render:function(){var buttonProperties,classes;classes=styler({disabled:this.props.disabled});buttonProperties={className:classes,role:"button"};if(!this.props.disabled){buttonProperties.onClick=this.props.onSubmit}return button(buttonProperties,i({className:"fa fa-"+this.props.icon}))}})},{"react/addons":38,underscore:202}],14:[function(require,module,exports){var ActionsBar,Breadcrumb,ProgressBar,React,Task,TaskActionCreator,TaskUtils,button,classer,div,h1,i,li,p,ref,span,styler,ul;React=require("react/addons");ref=React.DOM,div=ref.div,ul=ref.ul,li=ref.li,i=ref.i,h1=ref.h1,p=ref.p,button=ref.button,span=ref.span;TaskActionCreator=require("../actions/TaskActionCreator");TaskUtils=require("../utils/TaskUtil");Breadcrumb=React.createFactory(require("./breadcrumb"));Task=React.createFactory(require("./task"));ActionsBar=React.createFactory(require("./actions-bar"));ProgressBar=React.createFactory(require("./progress-bar"));styler=classer=React.addons.classSet;module.exports=React.createClass({displayName:"TaskList",render:function(){return div({role:"main"},Breadcrumb({selectedTags:this.props.selectedTags,searchQuery:this.props.searchQuery,isArchivedMode:this.props.isArchivedMode,onOpenMenu:this.props.onOpenMenu}),!this.props.isArchivedMode?this.getRenderTask({id:"new-task",key:"new-task",index:0,placeholder:this.generatePlaceholder(),defaultValue:this.generateDefaultValue(),isFocus:this.state.focusIndex===0}):void 0,ul({id:"task-list"},this.props.tasks.map(function(_this){return function(task,index){index=index+1;return _this.getRenderTask({key:"task-c"+task.cid,index:index,task:task,defaultValue:_this.generateDefaultValue(),isFocus:index===_this.state.focusIndex,isArchivedMode:_this.props.isArchivedMode})}}(this))),!this.props.isArchivedMode?ActionsBar({tasksDone:this.props.tasksDone,favoriteSearch:this.props.favoriteSearch,selectedTags:this.props.selectedTags}):void 0,!this.props.isArchivedMode?ProgressBar({tasks:this.props.tasks,tasksDone:this.props.tasksDone}):void 0)},getRenderTask:function(options){var currentTask,noop;currentTask=options.task||null;options.newTaskHandler=this.newTaskHandler.bind(this,currentTask);options.moveFocusUpHandler=this.moveFocusUpHandler.bind(this,currentTask);options.moveFocusDownHandler=this.moveFocusDownHandler.bind(this,currentTask);options.setFocusHandler=this.setFocusHandler;if(currentTask!=null){options.removeHandler=this.removeTaskHandler.bind(this,currentTask);options.saveHandler=this.saveTaskHandler.bind(this,currentTask);options.moveUpHandler=this.moveTaskUpHandler.bind(this,currentTask);options.moveDownHandler=this.moveTaskDownHandler.bind(this,currentTask);options.toggleStateHandler=this.toggleStateHandler.bind(this,currentTask);options.restoreTaskHandler=this.restoreTaskHandler.bind(this,currentTask)}else{noop=function(){};options.removeHandler=noop;options.saveHandler=noop;options.moveUpHandler=noop;options.moveDownHandler=noop;options.toggleStateHandler=noop;options.restoreTaskHandler=noop}return Task(options)},newTaskHandler:function(previousTask,content){if(content==null){content=""}if(content.length===0){content=this.generateDefaultValue()}return TaskActionCreator.createTask(content,previousTask)},removeTaskHandler:function(task){if(this.state.focusIndex!==1||this.props.tasks.length-1===0){this.moveFocusUpHandler()}return TaskActionCreator.removeTask(task)},saveTaskHandler:function(task,newContent){return TaskActionCreator.editTask(task,newContent)},moveTaskUpHandler:function(task){TaskActionCreator.moveUp(task);if(this.state.focusIndex!==1){return this.moveFocusUpHandler()}},moveTaskDownHandler:function(task){TaskActionCreator.moveDown(task);return this.moveFocusDownHandler()},toggleStateHandler:function(task,isDone){return TaskActionCreator.toggleState(task,isDone)},moveFocusUpHandler:function(){var newIndex;if(this.state.focusIndex>0){newIndex=this.state.focusIndex-1}else{newIndex=0}return this.setState({focusIndex:newIndex})},moveFocusDownHandler:function(){var listLength,newIndex;listLength=this.props.tasks.length;if(this.state.focusIndex<listLength){newIndex=this.state.focusIndex+1}else{newIndex=listLength}return this.setState({focusIndex:newIndex})},setFocusHandler:function(index){var newIndex;if(index!=null&&index>=0){newIndex=index}else{newIndex=-1}return this.setState({focusIndex:newIndex})},restoreTaskHandler:function(task){return TaskActionCreator.restoreTask(task)},getInitialState:function(){return{focusIndex:0}},generatePlaceholder:function(){var tagsList;tagsList=TaskUtils.buildTagsList(this.props.selectedTags,{tagPrefix:"#",regularSeparator:", ",lastSeparator:" "+t("and")+" "});if(tagsList.length>0){return t("form headline tags",{tagsList:tagsList})}else{return t("form headline")}},generateDefaultValue:function(){var tagsList;tagsList=TaskUtils.buildTagsList(this.props.selectedTags,{tagPrefix:"#"
});if(tagsList!==""){tagsList=tagsList+" "}return tagsList}})},{"../actions/TaskActionCreator":3,"../utils/TaskUtil":28,"./actions-bar":5,"./breadcrumb":9,"./progress-bar":12,"./task":15,"react/addons":38}],15:[function(require,module,exports){var $,KeyboardKeys,Options,React,TaskButton,ToggleCheckbox,classer,div,input,label,li,moment,p,ref1,ref2,indexOf=[].indexOf||function(item){for(var i=0,l=this.length;i<l;i++){if(i in this&&this[i]===item)return i}return-1};React=require("react/addons");$=require("jquery");moment=require("moment");ref1=React.DOM,div=ref1.div,li=ref1.li,input=ref1.input,p=ref1.p,label=ref1.label;ToggleCheckbox=React.createFactory(require("./toggle-checkbox"));TaskButton=React.createFactory(require("./task-button"));ref2=require("../constants/AppConstants"),KeyboardKeys=ref2.KeyboardKeys,Options=ref2.Options;classer=React.addons.classSet;module.exports=React.createClass({displayName:"Task",interval:null,render:function(){var completionDate,formattedDate,inputProperties,ref3,ref4,ref5,wrapperClasses;inputProperties={tabIndex:this.props.index+1,ref:"task-content",placeholder:this.props.placeholder||"",value:this.state.inputValue,onChange:this.onChange,onKeyUp:this.onKeyUp,onKeyDown:this.onKeyDown,onFocus:this.onFocus,onBlur:this.onBlur};wrapperClasses=classer({task:true,done:(ref3=this.props.task)!=null?ref3.done:void 0,"new-task":this.isNewTaskForm(),"is-creating":((ref4=this.props.task)!=null?ref4.id:void 0)==null});return li({className:wrapperClasses},this.isNewTaskForm()&&((ref5=this.props.task)!=null?ref5.id:void 0)==null?TaskButton({onSubmit:this.createNewTask,disabled:this.state.inputValue.length===0,icon:"plus"}):this.props.isArchivedMode?TaskButton({onSubmit:this.props.restoreTaskHandler,disabled:this.state.inputValue.length===0,icon:"mail-reply"}):ToggleCheckbox({id:this.props.index,isChecked:this.props.task.done,onToggle:this.onToggle}),div({className:"wrapper"},input(inputProperties),this.props.isArchivedMode?(this.props.task.completionDate!=null?(completionDate=moment(this.props.task.completionDate),formattedDate=completionDate.format(t("archived date format"))):formattedDate="",div({className:"todo-completionDate"},t("completed headline")+" "+formattedDate)):void 0))},componentDidMount:function(){return this.componentDidUpdate()},componentDidUpdate:function(){var index,node;if(this.props.isFocus){if(this.props.task!=null&&this.interval==null){this.startPeriodocalSave()}if(this.state.selectContent){node=this.refs["task-content"].getDOMNode();$(node).focus();index=node.value.length;node.setSelectionRange(0,index);return this.setState({selectContent:false})}}},getInitialState:function(){var ref3;return{inputValue:((ref3=this.props.task)!=null?ref3.description:void 0)||"",selectContent:true}},onChange:function(){var node;node=this.refs["task-content"].getDOMNode();return this.setState({inputValue:node.value})},onKeyDown:function(event){var authorizedComboKeys,comboKeyPressed,ctrlPressed,key,neutralKeys,node;node=this.refs["task-content"].getDOMNode();key=event.keyCode||event.charCode;ctrlPressed=event.ctrlKey||event.metaKey;comboKeyPressed=event.metaKey||event.ctrlKey||event.altKey;neutralKeys=[KeyboardKeys.BACKSPACE,KeyboardKeys.SPACE,KeyboardKeys.TAB,KeyboardKeys.ENTER,KeyboardKeys.ARROW_TOP,KeyboardKeys.ARROW_DOWN,KeyboardKeys.ARROW_LEFT,KeyboardKeys.ARROW_RIGHT];authorizedComboKeys=[KeyboardKeys.OSX_SHARP,KeyboardKeys.V];if(this.isNewTaskForm()&&node.value.length===0&&indexOf.call(neutralKeys,key)<0&&(!comboKeyPressed||indexOf.call(authorizedComboKeys,key)>=0)){return this.setState({inputValue:this.props.defaultValue})}else if(node.value.length===0&&key===KeyboardKeys.BACKSPACE){event.preventDefault();return this.props.removeHandler()}else if(key===KeyboardKeys.ARROW_TOP&&ctrlPressed){return this.props.moveUpHandler()}else if(key===KeyboardKeys.ARROW_DOWN&&ctrlPressed){return this.props.moveDownHandler()}else if(key===KeyboardKeys.ARROW_TOP){return this.props.moveFocusUpHandler()}else if(key===KeyboardKeys.ARROW_DOWN){return this.props.moveFocusDownHandler()}},onKeyUp:function(event){var key;key=event.keyCode||event.charCode;if(key===KeyboardKeys.ENTER){if(!this.props.isArchivedMode){return this.createNewTask()}}},onFocus:function(){if(!this.props.isFocus){this.props.setFocusHandler(this.props.index||0)}return this.startPeriodocalSave()},onBlur:function(){if(this.props.isFocus){this.props.setFocusHandler(null)}this.setState({selectContent:true});this.stopPeriodicalSave();return this.saveDescription()},onToggle:function(){return this.props.toggleStateHandler(!this.props.task.done)},createNewTask:function(){var content;content=this.props.task!=null?null:this.state.inputValue;this.props.newTaskHandler(content);if(this.props.task==null){this.setState({inputValue:""})}if(this.props.task!=null){return this.props.setFocusHandler(this.props.index+1)}},startPeriodocalSave:function(){if(this.interlval==null){return this.interval=setInterval(function(_this){return function(){return _this.saveDescription()}}(this),Options.SAVE_INTERVAL_TIME)}},saveDescription:function(){var node,ref,ref3;ref=this.refs["task-content"];if(ref!=null){node=ref.getDOMNode();if(node.value!==((ref3=this.props.task)!=null?ref3.description:void 0)){return this.props.saveHandler(node.value)}}},stopPeriodicalSave:function(){clearInterval(this.interval);return this.interval=null},componentWillUnmount:function(){return this.stopPeriodicalSave()},isNewTaskForm:function(){return this.props.placeholder!=null}})},{"../constants/AppConstants":17,"./task-button":13,"./toggle-checkbox":16,jquery:34,moment:35,"react/addons":38}],16:[function(require,module,exports){var React,_,classer,div,input,label,ref;React=require("react/addons");_=require("underscore");ref=React.DOM,div=ref.div,input=ref.input,label=ref.label;classer=React.addons.classSet;module.exports=React.createClass({displayName:"ToggleCheckbox",render:function(){var id,inputProperties;id="checkbox-"+this.props.id;inputProperties={id:id,type:"checkbox",checked:this.props.isChecked,onChange:this.props.onToggle};return div({role:"checkbox"},input(inputProperties),label({htmlFor:id}))}})},{"react/addons":38,underscore:202}],17:[function(require,module,exports){module.exports={ActionTypes:{SELECT_TAGS:"SELECT_TAGS",CREATE_TASK:"CREATE_TASK",EDIT_TASK:"EDIT_TASK",REMOVE_TASK:"REMOVE_TASK",REORDER_TASK:"REORDER_TASK",SELECT_SORT_CRITERION:"SELECT_SORT_CRITERION",SET_ARCHIVED_MODE:"SET_ARCHIVED_MODE",SET_SEARCH_QUERY:"SET_SEARCH_QUERY",ARCHIVE_TASK:"ARCHIVE_TASK",RESTORE_TASK:"RESTORE_TASK",SET_REINDEX_STATE:"SET_REINDEX_STATE",TOGGLE_FAVORITE_TAG:"TOGGLE_FAVORITE_TAG",MARK_SEARCH_AS_FAVORITE:"MARK_SEARCH_AS_FAVORITE"},KeyboardKeys:{ENTER:13,BACKSPACE:8,TAB:9,SPACE:32,ARROW_LEFT:37,ARROW_TOP:38,ARROW_RIGHT:39,ARROW_DOWN:40,V:86,OSX_SHARP:220},SortCriterions:{COUNT:"count",ALPHA:"alpha"},PayloadSources:{VIEW_ACTION:"VIEW_ACTION",SERVER_ACTION:"SERVER_ACTION"},Options:{SAVE_INTERVAL_TIME:5e3,MIN_STEP:Math.pow(10,4)}}},{}],18:[function(require,module,exports){var $,app;$=require("jquery");app=require("./application");$(function(){$.fn.spin=function(opts,color){var presets;presets={tiny:{lines:8,length:2,width:2,radius:3},small:{lines:8,length:1,width:2,radius:4},medium:{lines:10,length:4,width:3,radius:6},large:{lines:10,length:8,width:4,radius:8},extralarge:{lines:8,length:3,width:10,radius:20,top:30,left:50}};if(typeof Spinner!=="undefined"&&Spinner!==null){return this.each(function(){var $this,spinner;$this=$(this);spinner=$this.data("spinner");if(spinner!=null){spinner.stop();return $this.data("spinner",null)}else if(opts!==false){if(typeof opts==="string"){if(opts in presets){opts=presets[opts]}else{opts={}}if(color){opts.color=color}}spinner=new Spinner(opts);spinner.spin(this);return $this.data("spinner",spinner)}})}};return app.initialize()})},{"./application":4,jquery:34}],19:[function(require,module,exports){var Dispatcher,_lastID,_prefix,invariant;invariant=require("../invariant");_lastID=1;_prefix="ID_";module.exports=Dispatcher=Dispatcher=function(){function Dispatcher(){this._callbacks={};this._isPending={};this._isHandled={};this._isDispatching=false;this._pendingPayload=null}Dispatcher.prototype.register=function(callback){var id;id=_prefix+_lastID++;this._callbacks[id]=callback;return id};Dispatcher.prototype.unregister=function(id){invariant(this._callbacks[id],"Dispatcher.unregister(...): `%s` does not map to a registered callback.",id);return delete this._callbacks[id]};Dispatcher.prototype.waitFor=function(ids){var i,id,ii,ref,results;invariant(this._isDispatching,"Dispatcher.waitFor(...): Must be invoked while dispatching.");results=[];for(ii=i=0,ref=ids.length-1;i<=ref;ii=i+=1){id=ids[ii];if(this._isPending[id]){invariant(this._isHandled[id],"Dispatcher.waitFor(...): Circular dependency detected while waiting for `%s`.",id);continue}invariant(this._callbacks[id],"Dispatcher.waitFor(...): `%s` does not map to a registered callback.",id);results.push(this._invokeCallback(id))}return results};Dispatcher.prototype.dispatch=function(payload){var id,results;invariant(!this._isDispatching,"Dispatch.dispatch(...): Cannot dispatch in the middle of a dispatch.");this._startDispatching(payload);try{results=[];for(id in this._callbacks){if(this._isPending[id]){continue}results.push(this._invokeCallback(id))}return results}finally{this._stopDispatching()}};Dispatcher.prototype.isDispatching=function(){return this._isDispatching};Dispatcher.prototype._invokeCallback=function(id){this._isPending[id]=true;this._callbacks[id](this._pendingPayload);return this._isHandled[id]=true};Dispatcher.prototype._startDispatching=function(payload){var id;for(id in this._callbacks){this._isPending[id]=false;this._isHandled[id]=false}this._pendingPayload=payload;return this._isDispatching=true};Dispatcher.prototype._stopDispatching=function(){this._pendingPayload=null;return this._isDispatching=false};return Dispatcher}()},{"../invariant":20}],20:[function(require,module,exports){"use strict";var invariant=function(condition,format,a,b,c,d,e,f){if(__DEV__){if(format===undefined){throw new Error("invariant requires an error message argument")}}if(!condition){var error;if(format===undefined){error=new Error("Minified exception occurred; use the non-minified dev environment "+"for the full error message and additional helpful warnings.")}else{var args=[a,b,c,d,e,f];var argIndex=0;error=new Error("Invariant Violation: "+format.replace(/%s/g,function(){return args[argIndex++]}))}error.framesToPop=1;throw error}};module.exports=invariant},{}],21:[function(require,module,exports){var AppDispatcher,EventEmitter,Store,extend=function(child,parent){for(var key in parent){if(hasProp.call(parent,key))child[key]=parent[key]}function ctor(){this.constructor=child}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;return child},hasProp={}.hasOwnProperty;EventEmitter=require("events").EventEmitter;AppDispatcher=require("../../../AppDispatcher");module.exports=Store=function(superClass){var _addHandlers,_handlers,_nextUniqID,_processBinding;extend(Store,superClass);Store.prototype.uniqID=null;_nextUniqID=0;_handlers={};_addHandlers=function(type,callback){if(_handlers[this.uniqID]==null){_handlers[this.uniqID]={}}return _handlers[this.uniqID][type]=callback};_processBinding=function(){return this.dispatchToken=AppDispatcher.register(function(_this){return function(payload){var callback,ref,ref1,type,value;ref=payload.action,type=ref.type,value=ref.value;if((callback=(ref1=_handlers[_this.uniqID])!=null?ref1[type]:void 0)!=null){return callback.call(_this,value)}}}(this))};function Store(){Store.__super__.constructor.call(this);this.uniqID=_nextUniqID++;this.__bindHandlers(_addHandlers.bind(this));_processBinding.call(this)}Store.prototype.__bindHandlers=function(handle){if(__DEV__){throw new Error("The store "+this.constructor.name+" must define a `__bindHandlers` method")}};return Store}(EventEmitter)},{"../../../AppDispatcher":1,events:32}],22:[function(require,module,exports){module.exports={todo:"To-do",archived:"Archived",untagged:"untagged","all tasks":"All tasks","all archived tasks":"All archived tasks","untagged tasks":"Untagged tasks","tasks of":"Tasks related to","archived tasks of":"Archived tasks related to",and:"and","archived date format":"dddd, MMMM Do YYYY, h:mm:ss a","actions headline":"Actions: ","completed headline":"Completed on","restore button":"Restore","restore button?":"Restore?","done button":"Done","done button?":"Done?","todo button":"Todo","todo button?":"Todo?","new button":"New","archive button title":"Archive done tasks","form headline tags":"What's next about %{tagsList}?","form headline":"What's next?","reindexing message":"Server is reindexing all the tasks, please wait a little...","search tag input":"search...","match criterion no tag":"of criterion","match criterion with tag":"and of criterion","sort alpha":"Sort by tag name","sort numeric":"Sort by number of tasks","tag title":"%{label} (%{todoCount} to do, %{smart_count} done)","coming soon":"Coming soon!"}},{}],23:[function(require,module,exports){module.exports={todo:"À faire",archived:"Archivées",untagged:"sans étiquette","all tasks":"Toutes les tâches","all archived tasks":"Toutes les tâches archivées","untagged tasks":"Tâches sans étiquettes","tasks of":"Tâches de","archived tasks of":"Tâches archivées de",and:"et","archived date format":"DD/MM/YYYY [à] HH[h]mm","actions headline":"Actions : ","completed headline":"Effectuée le","restore button":"Restaurer","restore button?":"Restaurer ?","done button":"Fait","done button?":"Fait ?","todo button":"À faire","todo button?":"À faire ?","new button":"Ajouter","archive button title":"Archiver les tâches faites","form headline tags":"Que devez-vous faire à propos de %{tagsList} ?","form headline":"Que devez-vous faire ?","reindexing message":"Le serveur est en train de ré-indexer les tâches, merci de patienter…","search tag input":"recherche…","match criterion no tag":"correspondant au critère","match criterion with tag":"et au critère","sort alpha":"Tri par étiquette","sort numeric":"Tri par nombre de tâches","tag title":"%{label} (%{todoCount} à faire, %{smart_count} faite) |||| %{label} (%{todoCount} à faire, %{smart_count} faites)","coming soon":"Bientôt disponible !"}},{}],24:[function(require,module,exports){var StoreWatchMixin;module.exports=StoreWatchMixin=function(stores){return{componentDidMount:function(){return stores.forEach(function(_this){return function(store){return store.on("change",_this._setStateFromStores)}}(this))},componentWillUnmount:function(){return stores.forEach(function(_this){return function(store){return store.removeListener("change",_this._setStateFromStores)}}(this))},getInitialState:function(){return this.getStateFromStores()},_setStateFromStores:function(){return this.setState(this.getStateFromStores())}}}},{}],25:[function(require,module,exports){var $,App,Backbone,React,Router,TagActionCreator,TaskActionCreator,extend=function(child,parent){for(var key in parent){if(hasProp.call(parent,key))child[key]=parent[key]}function ctor(){this.constructor=child}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;return child},hasProp={}.hasOwnProperty;window.__DEV__=true;Backbone=require("backbone");React=require("react/addons");$=require("jquery");TaskActionCreator=require("./actions/TaskActionCreator");TagActionCreator=require("./actions/TagActionCreator");TaskActionCreator=require("./actions/TaskActionCreator");App=React.createFactory(require("./components/application"));Router=function(superClass){extend(Router,superClass);function Router(){return Router.__super__.constructor.apply(this,arguments)}Router.prototype.routes={"":"main",archived:"archived","search/:query":"mainSearch","todoByTags/*tags/;search/:query":"todoByTagsWithSearch","todoByTags/;search/:query":"todoByTagsWithSearch","todoByTags/*tags":"todoByTags","archivedByTags/*tags":"archivedByTags"};Router.prototype.main=function(followUp){if(followUp==null){followUp=false}TaskActionCreator.setSearchQuery(null);TaskActionCreator.setArchivedMode(false);TagActionCreator.selectTags(null);return React.render(App(),$("div#mount-point")[0])};Router.prototype.mainSearch=function(query){TaskActionCreator.setSearchQuery(query);TaskActionCreator.setArchivedMode(false);TagActionCreator.selectTags(null);return React.render(App(),$("div#mount-point")[0])};Router.prototype.archived=function(){TaskActionCreator.setSearchQuery(null);TaskActionCreator.setArchivedMode(true);TagActionCreator.selectTags(null);return React.render(App(),$("div#mount-point")[0])};Router.prototype.byTags=function(viewType,listView,tags,searchQuery,isArchived){if(tags!=null){tags=tags.split("/");if(tags[tags.length-1].length===0){tags.splice(tags.length-1)}}else{tags=[]}TaskActionCreator.setArchivedMode(isArchived);TagActionCreator.selectTags(tags);TaskActionCreator.setSearchQuery(searchQuery);return React.render(App(),$("div#mount-point")[0])};Router.prototype.todoByTags=function(tags){return this.byTags("#tobedone",this.taskList,tags,null,false)};Router.prototype.todoByTagsWithSearch=function(tags,query){if(query==null){query=tags;tags=null}return this.byTags("#tobedone",this.taskList,tags,query)};Router.prototype.archivedByTags=function(tags){return this.byTags("#archived",this.archivedTaskList,tags,null,true)};Router.prototype.goToDefault=function(favoriteSearch){var currentRoute,url;currentRoute=Backbone.history.getFragment();if(currentRoute.length===0&&favoriteSearch!=null){url=favoriteSearch.map(function(tag){var prefix;if(tag.isExcluded){prefix="!"}else{prefix=""}return""+prefix+tag.label+"/"});url="#todoByTags/"+url;return this.navigate(url,true)}};return Router}(Backbone.Router);module.exports=new Router},{"./actions/TagActionCreator":2,"./actions/TaskActionCreator":3,"./components/application":7,backbone:31,jquery:34,"react/addons":38}],26:[function(require,module,exports){var ActionTypes,SortCriterions,Store,TagStore,TaskStore,_,ref,extend=function(child,parent){for(var key in parent){if(hasProp.call(parent,key))child[key]=parent[key]}function ctor(){this.constructor=child}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;return child},hasProp={}.hasOwnProperty,indexOf=[].indexOf||function(item){for(var i=0,l=this.length;i<l;i++){if(i in this&&this[i]===item)return i}return-1};_=require("underscore");Store=require("../libs/flux/store/Store");TaskStore=require("./TaskStore");ref=require("../constants/AppConstants"),ActionTypes=ref.ActionTypes,SortCriterions=ref.SortCriterions;TagStore=function(superClass){var _favoriteSearch,_favoriteTags,_selectedTags,_sortCriterion,fromLocalStorage,json;extend(TagStore,superClass);function TagStore(){return TagStore.__super__.constructor.apply(this,arguments)}_selectedTags=null;_favoriteTags=window.favoriteTags||[];json=localStorage.getItem("tasky.favorite_search");_favoriteSearch=JSON.parse(json);fromLocalStorage=localStorage.getItem("sort-criterion");_sortCriterion=fromLocalStorage||SortCriterions.COUNT;TagStore.prototype.__bindHandlers=function(handle){handle(ActionTypes.SELECT_TAGS,function(tags){_selectedTags=tags!=null?tags.map(function(tag){var isExcluded,label;isExcluded=tag.indexOf("!")!==-1;label=tag.replace("!","");return{label:label,isExcluded:isExcluded}}):void 0;return this.emit("change")});handle(ActionTypes.SELECT_SORT_CRITERION,function(criterion){_sortCriterion=criterion;return this.emit("change")});handle(ActionTypes.TOGGLE_FAVORITE_TAG,function(tag){var tagIndex;tagIndex=_favoriteTags.indexOf(tag);if(tagIndex===-1){_favoriteTags.push(tag)}else{_favoriteTags.splice(tagIndex,1)}return this.emit("change")});return handle(ActionTypes.MARK_SEARCH_AS_FAVORITE,function(selectedTags){_favoriteSearch=selectedTags;return this.emit("change")})};TagStore.prototype.getSelected=function(){return _selectedTags};TagStore.prototype.getSelectedNames=function(){return _selectedTags!=null?_selectedTags.map(function(tag){return tag.label}):void 0};TagStore.prototype.getSortCriterion=function(){return _sortCriterion};TagStore.prototype.getFavoriteTags=function(){return _favoriteTags};TagStore.prototype.getFavoriteSearch=function(){return _favoriteSearch};TagStore.prototype.getTree=function(){var aTree,branch,buildTree,count,depth,depths,factor,firstCriterion,i,j,len,maxDepth,ref1,ref2,ref3,secondCriterion,selectedTagNames,tag,tree;selectedTagNames=this.getSelectedNames();maxDepth=(selectedTagNames!=null?selectedTagNames.length:void 0)||0;tree=[];for(depth=i=0,ref1=maxDepth;i<=ref1;depth=i+=1){tree.push({})}buildTree=function(depth,list,isDone,excludeList){var uniqList;if(excludeList==null){excludeList=[]}uniqList=_.uniq(list);return uniqList.forEach(function(tag){var base;if(indexOf.call(excludeList,tag)<0){if((base=tree[depth])[tag]==null){base[tag]={global:0,done:0}}tree[depth][tag]["global"]++;if(isDone){return tree[depth][tag]["done"]++}}})};TaskStore.getAll().forEach(function(task){var intersection,isDone,j,processedSelection,ref2,results,tagsOfTask;tagsOfTask=task.tags;isDone=task.done;buildTree(0,tagsOfTask,isDone);results=[];for(depth=j=1,ref2=maxDepth;j<=ref2;depth=j+=1){processedSelection=selectedTagNames!=null?selectedTagNames.slice(0,depth):void 0;intersection=_.intersection(processedSelection,tagsOfTask);if(intersection.length===processedSelection.length){results.push(buildTree(depth,tagsOfTask,isDone,processedSelection))}else{results.push(void 0)}}return results});aTree=[];if(_sortCriterion==="count"){ref2=["count","label",1],firstCriterion=ref2[0],secondCriterion=ref2[1],factor=ref2[2]}else if(_sortCriterion==="alpha"){ref3=["label","count",-1],firstCriterion=ref3[0],secondCriterion=ref3[1],factor=ref3[2]}for(j=0,len=tree.length;j<len;j++){branch=tree[j];depths=[];for(tag in branch){count=branch[tag];depths.push({label:tag,count:count.global,doneCount:count.done,isFavorite:_favoriteTags.indexOf(tag)!==-1})}depths.sort(function(a,b){var aFirst,aSecond,bFirst,bSecond;if(a.isFavorite&&!b.isFavorite){return-1}else if(!a.isFavorite&&b.isFavorite){return 1}else{aFirst=a[firstCriterion];bFirst=b[firstCriterion];if(aFirst>bFirst){return-1*factor}else if(aFirst<bFirst){return 1*factor}else{aSecond=a[secondCriterion];bSecond=b[secondCriterion];if(aSecond>bSecond){return-1*factor}else if(aSecond<bSecond){return 1*factor}else{return 0}}}});aTree.push(depths)}return aTree};return TagStore}(Store);module.exports=new TagStore},{"../constants/AppConstants":17,"../libs/flux/store/Store":21,"./TaskStore":27,underscore:202}],27:[function(require,module,exports){var ActionTypes,Store,TaskStore,TaskUtils,_,extend=function(child,parent){for(var key in parent){if(hasProp.call(parent,key))child[key]=parent[key]}function ctor(){this.constructor=child}ctor.prototype=parent.prototype;child.prototype=new ctor;child.__super__=parent.prototype;return child},hasProp={}.hasOwnProperty;_=require("underscore");Store=require("../libs/flux/store/Store");TaskUtils=require("../utils/TaskUtil");ActionTypes=require("../constants/AppConstants").ActionTypes;TaskStore=function(superClass){var _archivedMode,_archivedTasks,_archivedTasksByCid,_cache,_cid,_getTaskLists,_invalidCache,_isReindexing,_searchQuery,_tasks,_tasksByCid,j,k,len,len1,task;extend(TaskStore,superClass);function TaskStore(){return TaskStore.__super__.constructor.apply(this,arguments)}_cid=0;_archivedMode=false;_archivedTasks=window.archivedTasks;_tasks=window.tasks;_searchQuery=null;_isReindexing=false;_cache={};_archivedTasks.sort(function(a,b){if(a.completionDate>b.completionDate){return 1}else if(a.completionDate<b.completionDate){return-1}else{return 0}});_tasks.sort(function(a,b){if(a.order>b.order){return 1}else if(a.order<b.order){return-1}else{return 0}});_tasksByCid={};for(j=0,len=_tasks.length;j<len;j++){task=_tasks[j];task.cid=_cid++;_tasksByCid[task.cid]=task}_archivedTasksByCid={};for(k=0,len1=_archivedTasks.length;k<len1;k++){task=_archivedTasks[k];task.cid=_cid++;_archivedTasksByCid[task.cid]=task}_getTaskLists=function(){var tasksList,tasksListByCid;if(_archivedMode){tasksList=_archivedTasks;tasksListByCid=_archivedTasksByCid}else{tasksList=_tasks;tasksListByCid=_tasksByCid}return{tasksList:tasksList,tasksListByCid:tasksListByCid}};_invalidCache=function(task){var cacheKey,cacheKeys,i,l,len2,results,tags;tags=task.tags;cacheKeys=Object.keys(_cache);results=[];for(l=0,len2=cacheKeys.length;l<len2;l++){cacheKey=cacheKeys[l];i=0;while(i<tags.length&&cacheKey.indexOf(tags[i])===-1){i++}if(cacheKey.indexOf(tags[i])!==-1){results.push(delete _cache[cacheKey])}else{results.push(void 0)}}return results};TaskStore.prototype.__bindHandlers=function(handle){handle(ActionTypes.CREATE_TASK,function(payload){var nextIndex,rawTask;nextIndex=payload.nextIndex,rawTask=payload.rawTask;_tasks.splice(nextIndex,0,rawTask);_tasksByCid[rawTask.cid]=rawTask;_invalidCache(rawTask);return this.emit("change")});handle(ActionTypes.REMOVE_TASK,function(task){var index,ref,tasksList,tasksListByCid;ref=_getTaskLists(),tasksList=ref.tasksList,tasksListByCid=ref.tasksListByCid;index=tasksList.indexOf(task);tasksList.splice(index,1);delete tasksListByCid[task.cid];_invalidCache(task);return this.emit("change")});handle(ActionTypes.UPDATE_TASK,function(payload){var changes,cid,field,ref,tasksList,tasksListByCid,value;ref=_getTaskLists(),tasksList=ref.tasksList,tasksListByCid=ref.tasksListByCid;cid=payload.cid,changes=payload.changes;task=tasksListByCid[cid];_invalidCache(task);for(field in changes){value=changes[field];task[field]=value}if(changes.tags!=null){_invalidCache(task)}return this.emit("change")});handle(ActionTypes.REORDER_TASK,function(payload){var changedPiece,index,oldIndex,ref,task1,task2,tasksList,tasksListByCid;ref=_getTaskLists(),tasksList=ref.tasksList,tasksListByCid=ref.tasksListByCid;changedPiece=payload.changedPiece,index=payload.index,oldIndex=payload.oldIndex,task=payload.task;task1=changedPiece[0],task2=changedPiece[1];index=Math.max(index,0);tasksList.splice(oldIndex,1);tasksList.splice(index,0,task);_invalidCache(task);return this.emit("change")});handle(ActionTypes.SET_ARCHIVED_MODE,function(isArchived){_archivedMode=isArchived;return this.emit("change")});handle(ActionTypes.SET_SEARCH_QUERY,function(searchQuery){_searchQuery=searchQuery;return this.emit("change")});handle(ActionTypes.ARCHIVE_TASK,function(cid){var index;task=_tasksByCid[cid];if(task!=null){index=_tasks.indexOf(task);delete _tasksByCid[cid];_tasks.splice(index,1);task.isArchived=true;_archivedTasksByCid[cid]=task;index=_.sortedIndex(_archivedTasks,task,function(task){return-new Date(task.completionDate).getTime()});_archivedTasks.splice(index,0,task);_invalidCache(task);return this.emit("change")}});handle(ActionTypes.RESTORE_TASK,function(cid){var index;task=_archivedTasksByCid[cid];if(task!=null){index=_archivedTasks.indexOf(task);delete _archivedTasksByCid[cid];_archivedTasks.splice(index,1);task.isArchived=false;task.done=false;_tasksByCid[cid]=task;index=_.sortedIndex(_tasks,task,function(task){return task.order});_tasks.splice(index,0,task);_invalidCache(task);return this.emit("change")}});return handle(ActionTypes.SET_REINDEX_STATE,function(isReindexing){_isReindexing=isReindexing;return this.emit("change")})};TaskStore.prototype.getAll=function(){var tasksList;tasksList=_getTaskLists().tasksList;return tasksList};TaskStore.prototype.getUntagged=function(){var tasksList;tasksList=_getTaskLists().tasksList;return tasksList.filter(function(task){return task.tags.length===0})};TaskStore.prototype.getByTags=function(tags){var cacheKey,cachedList,excludedTags,filteredTasksList,includedTags,mapValue,noInclusion,regex,tasksList;tasksList=_getTaskLists().tasksList;if(tags!=null&&tags.length===0){return tasksList.filter(function(task){return task.tags.length===0})}else{filteredTasksList=tasksList;if(tags!=null){mapValue=function(tag){return tag.label};includedTags=tags.filter(function(tag){return!tag.isExcluded}).map(mapValue);noInclusion=includedTags.length===0;excludedTags=tags.filter(function(tag){return tag.isExcluded}).map(mapValue);cacheKey=[].concat(tags).sort(function(a,b){if(a.label>b.label){return 1}else if(a.label>b.label){return-1}else{return 0}}).map(function(tag){var prefix;prefix=tag.isExcluded?"!":"";return""+prefix+tag.label}).join("#");cachedList=_cache[cacheKey];if(cachedList!=null){filteredTasksList=cachedList}else{filteredTasksList=filteredTasksList.filter(function(task){return(TaskUtils.containsTags(task,includedTags)||noInclusion)&&TaskUtils.doesntContainsTags(task,excludedTags)});_cache[cacheKey]=filteredTasksList}}if(_searchQuery!=null){regex=new RegExp(_searchQuery,"i");filteredTasksList=filteredTasksList.filter(function(task){return regex.test(task.description)})}return filteredTasksList}};TaskStore.prototype.getNextCid=function(){return _cid++};TaskStore.prototype.isArchivedMode=function(){return _archivedMode};TaskStore.prototype.getNumTasks=function(){return _tasks.length};TaskStore.prototype.getNumArchivedTasks=function(){return _archivedTasks.length};TaskStore.prototype.getSearchQuery=function(){return _searchQuery};TaskStore.prototype.isReindexing=function(){return _isReindexing};return TaskStore}(Store);module.exports=new TaskStore},{"../constants/AppConstants":17,"../libs/flux/store/Store":21,"../utils/TaskUtil":28,underscore:202}],28:[function(require,module,exports){var _,regex;_=require("underscore");regex=/(^|\s)#([\w\d\-_\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]+)(?=\s|$)/g;module.exports.containsTags=function(task,tags){var lowerCasedTags;if(tags.length===0){return task.tags.length===0}else{lowerCasedTags=task.tags.map(function(tag){return tag.toLowerCase()});return _.every(tags,_.partial(_.contains,lowerCasedTags))}};module.exports.doesntContainsTags=function(task,tags){var lowerCasedTags;if(tags.length===0){return true}else{lowerCasedTags=task.tags.map(function(tag){return tag.toLowerCase()});return!_.some(tags,_.partial(_.contains,lowerCasedTags))}};module.exports.extractTags=function(desc){var tags;tags=desc.match(regex);tags=_.map(tags,function(tag){return tag.trim().replace("#","").toLowerCase()});tags=_.uniq(tags);return tags};module.exports.getNewOrder=function(previousTask,nextTask){var lowBoundary,order,step,topBoundary;topBoundary=nextTask!=null?nextTask.order:Number.MAX_VALUE;lowBoundary=previousTask!=null?previousTask.order:0;step=(topBoundary-lowBoundary)/2;order=lowBoundary+step;return{order:order,step:step}};module.exports.buildTagsList=function(tags,options){var includedTags,lastSeparator,regularSeparator,tagPrefix,tagsList;if(options==null){options={}}tagPrefix=options.tagPrefix||"";regularSeparator=options.regularSeparator||" ";lastSeparator=options.lastSeparator||" ";if(tags==null){return""}tagsList="";includedTags=tags.filter(function(tag){return!tag.isExcluded}).map(function(tag){return tag.label});includedTags.forEach(function(tag){if(includedTags.indexOf(tag)===0){return tagsList=""+tagPrefix+tag}else if(includedTags.indexOf(tag)===includedTags.length-1){return tagsList=""+tagsList+lastSeparator+tagPrefix+tag}else{return tagsList=""+tagsList+regularSeparator+tagPrefix+tag}});return tagsList}},{underscore:202}],29:[function(require,module,exports){var request;request=require("superagent");module.exports={create:function(rawTask,callback){return request.post("tasks").send(rawTask).set("Accept","application/json").end(function(res){if(res.ok){return callback(null,res.body)}else{return callback("Something went wrong -- "+res.body)}})},update:function(taskID,attributes,callback){return request.put("tasks/"+taskID).send(attributes).set("Accept","application/json").end(function(res){if(res.ok){return callback(null,res.body)}else{return callback("Something went wrong -- "+res.body)}})},remove:function(taskID,callback){return request.del("tasks/"+taskID).set("Accept","application/json").end(function(res){if(res.ok){return callback(null)}else{return callback("Something went wrong -- "+res.body);

}})},reindex:function(callback){return request.post("tasks/reindex").end(function(res){if(res.ok){return callback(null,res.body)}else{return callback("Something went wrong -- "+res.body)}})},markTagAsFavorite:function(label,callback){return request.post("tags").send({label:label}).end(function(res){if(res.ok){return callback(null)}else{return callback("Something went wrong -- "+res.body)}})},unmarkTagAsFavorite:function(label,callback){return request.del("tags").send({label:label}).end(function(res){if(res.ok){return callback(null)}else{return callback("Something went wrong -- "+res.body)}})},markSearchAsFavorite:function(search){var json;json=JSON.stringify(search);return localStorage.setItem("tasky.favorite_search",json)}}},{superagent:199}],30:[function(require,module,exports){(function(process){(function(){var async={};var root,previous_async;root=this;if(root!=null){previous_async=root.async}async.noConflict=function(){root.async=previous_async;return async};function only_once(fn){var called=false;return function(){if(called)throw new Error("Callback was already called.");called=true;fn.apply(root,arguments)}}var _toString=Object.prototype.toString;var _isArray=Array.isArray||function(obj){return _toString.call(obj)==="[object Array]"};var _each=function(arr,iterator){if(arr.forEach){return arr.forEach(iterator)}for(var i=0;i<arr.length;i+=1){iterator(arr[i],i,arr)}};var _map=function(arr,iterator){if(arr.map){return arr.map(iterator)}var results=[];_each(arr,function(x,i,a){results.push(iterator(x,i,a))});return results};var _reduce=function(arr,iterator,memo){if(arr.reduce){return arr.reduce(iterator,memo)}_each(arr,function(x,i,a){memo=iterator(memo,x,i,a)});return memo};var _keys=function(obj){if(Object.keys){return Object.keys(obj)}var keys=[];for(var k in obj){if(obj.hasOwnProperty(k)){keys.push(k)}}return keys};if(typeof process==="undefined"||!process.nextTick){if(typeof setImmediate==="function"){async.nextTick=function(fn){setImmediate(fn)};async.setImmediate=async.nextTick}else{async.nextTick=function(fn){setTimeout(fn,0)};async.setImmediate=async.nextTick}}else{async.nextTick=process.nextTick;if(typeof setImmediate!=="undefined"){async.setImmediate=function(fn){setImmediate(fn)}}else{async.setImmediate=async.nextTick}}async.each=function(arr,iterator,callback){callback=callback||function(){};if(!arr.length){return callback()}var completed=0;_each(arr,function(x){iterator(x,only_once(done))});function done(err){if(err){callback(err);callback=function(){}}else{completed+=1;if(completed>=arr.length){callback()}}}};async.forEach=async.each;async.eachSeries=function(arr,iterator,callback){callback=callback||function(){};if(!arr.length){return callback()}var completed=0;var iterate=function(){iterator(arr[completed],function(err){if(err){callback(err);callback=function(){}}else{completed+=1;if(completed>=arr.length){callback()}else{iterate()}}})};iterate()};async.forEachSeries=async.eachSeries;async.eachLimit=function(arr,limit,iterator,callback){var fn=_eachLimit(limit);fn.apply(null,[arr,iterator,callback])};async.forEachLimit=async.eachLimit;var _eachLimit=function(limit){return function(arr,iterator,callback){callback=callback||function(){};if(!arr.length||limit<=0){return callback()}var completed=0;var started=0;var running=0;(function replenish(){if(completed>=arr.length){return callback()}while(running<limit&&started<arr.length){started+=1;running+=1;iterator(arr[started-1],function(err){if(err){callback(err);callback=function(){}}else{completed+=1;running-=1;if(completed>=arr.length){callback()}else{replenish()}}})}})()}};var doParallel=function(fn){return function(){var args=Array.prototype.slice.call(arguments);return fn.apply(null,[async.each].concat(args))}};var doParallelLimit=function(limit,fn){return function(){var args=Array.prototype.slice.call(arguments);return fn.apply(null,[_eachLimit(limit)].concat(args))}};var doSeries=function(fn){return function(){var args=Array.prototype.slice.call(arguments);return fn.apply(null,[async.eachSeries].concat(args))}};var _asyncMap=function(eachfn,arr,iterator,callback){arr=_map(arr,function(x,i){return{index:i,value:x}});if(!callback){eachfn(arr,function(x,callback){iterator(x.value,function(err){callback(err)})})}else{var results=[];eachfn(arr,function(x,callback){iterator(x.value,function(err,v){results[x.index]=v;callback(err)})},function(err){callback(err,results)})}};async.map=doParallel(_asyncMap);async.mapSeries=doSeries(_asyncMap);async.mapLimit=function(arr,limit,iterator,callback){return _mapLimit(limit)(arr,iterator,callback)};var _mapLimit=function(limit){return doParallelLimit(limit,_asyncMap)};async.reduce=function(arr,memo,iterator,callback){async.eachSeries(arr,function(x,callback){iterator(memo,x,function(err,v){memo=v;callback(err)})},function(err){callback(err,memo)})};async.inject=async.reduce;async.foldl=async.reduce;async.reduceRight=function(arr,memo,iterator,callback){var reversed=_map(arr,function(x){return x}).reverse();async.reduce(reversed,memo,iterator,callback)};async.foldr=async.reduceRight;var _filter=function(eachfn,arr,iterator,callback){var results=[];arr=_map(arr,function(x,i){return{index:i,value:x}});eachfn(arr,function(x,callback){iterator(x.value,function(v){if(v){results.push(x)}callback()})},function(err){callback(_map(results.sort(function(a,b){return a.index-b.index}),function(x){return x.value}))})};async.filter=doParallel(_filter);async.filterSeries=doSeries(_filter);async.select=async.filter;async.selectSeries=async.filterSeries;var _reject=function(eachfn,arr,iterator,callback){var results=[];arr=_map(arr,function(x,i){return{index:i,value:x}});eachfn(arr,function(x,callback){iterator(x.value,function(v){if(!v){results.push(x)}callback()})},function(err){callback(_map(results.sort(function(a,b){return a.index-b.index}),function(x){return x.value}))})};async.reject=doParallel(_reject);async.rejectSeries=doSeries(_reject);var _detect=function(eachfn,arr,iterator,main_callback){eachfn(arr,function(x,callback){iterator(x,function(result){if(result){main_callback(x);main_callback=function(){}}else{callback()}})},function(err){main_callback()})};async.detect=doParallel(_detect);async.detectSeries=doSeries(_detect);async.some=function(arr,iterator,main_callback){async.each(arr,function(x,callback){iterator(x,function(v){if(v){main_callback(true);main_callback=function(){}}callback()})},function(err){main_callback(false)})};async.any=async.some;async.every=function(arr,iterator,main_callback){async.each(arr,function(x,callback){iterator(x,function(v){if(!v){main_callback(false);main_callback=function(){}}callback()})},function(err){main_callback(true)})};async.all=async.every;async.sortBy=function(arr,iterator,callback){async.map(arr,function(x,callback){iterator(x,function(err,criteria){if(err){callback(err)}else{callback(null,{value:x,criteria:criteria})}})},function(err,results){if(err){return callback(err)}else{var fn=function(left,right){var a=left.criteria,b=right.criteria;return a<b?-1:a>b?1:0};callback(null,_map(results.sort(fn),function(x){return x.value}))}})};async.auto=function(tasks,callback){callback=callback||function(){};var keys=_keys(tasks);var remainingTasks=keys.length;if(!remainingTasks){return callback()}var results={};var listeners=[];var addListener=function(fn){listeners.unshift(fn)};var removeListener=function(fn){for(var i=0;i<listeners.length;i+=1){if(listeners[i]===fn){listeners.splice(i,1);return}}};var taskComplete=function(){remainingTasks--;_each(listeners.slice(0),function(fn){fn()})};addListener(function(){if(!remainingTasks){var theCallback=callback;callback=function(){};theCallback(null,results)}});_each(keys,function(k){var task=_isArray(tasks[k])?tasks[k]:[tasks[k]];var taskCallback=function(err){var args=Array.prototype.slice.call(arguments,1);if(args.length<=1){args=args[0]}if(err){var safeResults={};_each(_keys(results),function(rkey){safeResults[rkey]=results[rkey]});safeResults[k]=args;callback(err,safeResults);callback=function(){}}else{results[k]=args;async.setImmediate(taskComplete)}};var requires=task.slice(0,Math.abs(task.length-1))||[];var ready=function(){return _reduce(requires,function(a,x){return a&&results.hasOwnProperty(x)},true)&&!results.hasOwnProperty(k)};if(ready()){task[task.length-1](taskCallback,results)}else{var listener=function(){if(ready()){removeListener(listener);task[task.length-1](taskCallback,results)}};addListener(listener)}})};async.retry=function(times,task,callback){var DEFAULT_TIMES=5;var attempts=[];if(typeof times==="function"){callback=task;task=times;times=DEFAULT_TIMES}times=parseInt(times,10)||DEFAULT_TIMES;var wrappedTask=function(wrappedCallback,wrappedResults){var retryAttempt=function(task,finalAttempt){return function(seriesCallback){task(function(err,result){seriesCallback(!err||finalAttempt,{err:err,result:result})},wrappedResults)}};while(times){attempts.push(retryAttempt(task,!(times-=1)))}async.series(attempts,function(done,data){data=data[data.length-1];(wrappedCallback||callback)(data.err,data.result)})};return callback?wrappedTask():wrappedTask};async.waterfall=function(tasks,callback){callback=callback||function(){};if(!_isArray(tasks)){var err=new Error("First argument to waterfall must be an array of functions");return callback(err)}if(!tasks.length){return callback()}var wrapIterator=function(iterator){return function(err){if(err){callback.apply(null,arguments);callback=function(){}}else{var args=Array.prototype.slice.call(arguments,1);var next=iterator.next();if(next){args.push(wrapIterator(next))}else{args.push(callback)}async.setImmediate(function(){iterator.apply(null,args)})}}};wrapIterator(async.iterator(tasks))()};var _parallel=function(eachfn,tasks,callback){callback=callback||function(){};if(_isArray(tasks)){eachfn.map(tasks,function(fn,callback){if(fn){fn(function(err){var args=Array.prototype.slice.call(arguments,1);if(args.length<=1){args=args[0]}callback.call(null,err,args)})}},callback)}else{var results={};eachfn.each(_keys(tasks),function(k,callback){tasks[k](function(err){var args=Array.prototype.slice.call(arguments,1);if(args.length<=1){args=args[0]}results[k]=args;callback(err)})},function(err){callback(err,results)})}};async.parallel=function(tasks,callback){_parallel({map:async.map,each:async.each},tasks,callback)};async.parallelLimit=function(tasks,limit,callback){_parallel({map:_mapLimit(limit),each:_eachLimit(limit)},tasks,callback)};async.series=function(tasks,callback){callback=callback||function(){};if(_isArray(tasks)){async.mapSeries(tasks,function(fn,callback){if(fn){fn(function(err){var args=Array.prototype.slice.call(arguments,1);if(args.length<=1){args=args[0]}callback.call(null,err,args)})}},callback)}else{var results={};async.eachSeries(_keys(tasks),function(k,callback){tasks[k](function(err){var args=Array.prototype.slice.call(arguments,1);if(args.length<=1){args=args[0]}results[k]=args;callback(err)})},function(err){callback(err,results)})}};async.iterator=function(tasks){var makeCallback=function(index){var fn=function(){if(tasks.length){tasks[index].apply(null,arguments)}return fn.next()};fn.next=function(){return index<tasks.length-1?makeCallback(index+1):null};return fn};return makeCallback(0)};async.apply=function(fn){var args=Array.prototype.slice.call(arguments,1);return function(){return fn.apply(null,args.concat(Array.prototype.slice.call(arguments)))}};var _concat=function(eachfn,arr,fn,callback){var r=[];eachfn(arr,function(x,cb){fn(x,function(err,y){r=r.concat(y||[]);cb(err)})},function(err){callback(err,r)})};async.concat=doParallel(_concat);async.concatSeries=doSeries(_concat);async.whilst=function(test,iterator,callback){if(test()){iterator(function(err){if(err){return callback(err)}async.whilst(test,iterator,callback)})}else{callback()}};async.doWhilst=function(iterator,test,callback){iterator(function(err){if(err){return callback(err)}var args=Array.prototype.slice.call(arguments,1);if(test.apply(null,args)){async.doWhilst(iterator,test,callback)}else{callback()}})};async.until=function(test,iterator,callback){if(!test()){iterator(function(err){if(err){return callback(err)}async.until(test,iterator,callback)})}else{callback()}};async.doUntil=function(iterator,test,callback){iterator(function(err){if(err){return callback(err)}var args=Array.prototype.slice.call(arguments,1);if(!test.apply(null,args)){async.doUntil(iterator,test,callback)}else{callback()}})};async.queue=function(worker,concurrency){if(concurrency===undefined){concurrency=1}function _insert(q,data,pos,callback){if(!q.started){q.started=true}if(!_isArray(data)){data=[data]}if(data.length==0){return async.setImmediate(function(){if(q.drain){q.drain()}})}_each(data,function(task){var item={data:task,callback:typeof callback==="function"?callback:null};if(pos){q.tasks.unshift(item)}else{q.tasks.push(item)}if(q.saturated&&q.tasks.length===q.concurrency){q.saturated()}async.setImmediate(q.process)})}var workers=0;var q={tasks:[],concurrency:concurrency,saturated:null,empty:null,drain:null,started:false,paused:false,push:function(data,callback){_insert(q,data,false,callback)},kill:function(){q.drain=null;q.tasks=[]},unshift:function(data,callback){_insert(q,data,true,callback)},process:function(){if(!q.paused&&workers<q.concurrency&&q.tasks.length){var task=q.tasks.shift();if(q.empty&&q.tasks.length===0){q.empty()}workers+=1;var next=function(){workers-=1;if(task.callback){task.callback.apply(task,arguments)}if(q.drain&&q.tasks.length+workers===0){q.drain()}q.process()};var cb=only_once(next);worker(task.data,cb)}},length:function(){return q.tasks.length},running:function(){return workers},idle:function(){return q.tasks.length+workers===0},pause:function(){if(q.paused===true){return}q.paused=true;q.process()},resume:function(){if(q.paused===false){return}q.paused=false;q.process()}};return q};async.priorityQueue=function(worker,concurrency){function _compareTasks(a,b){return a.priority-b.priority}function _binarySearch(sequence,item,compare){var beg=-1,end=sequence.length-1;while(beg<end){var mid=beg+(end-beg+1>>>1);if(compare(item,sequence[mid])>=0){beg=mid}else{end=mid-1}}return beg}function _insert(q,data,priority,callback){if(!q.started){q.started=true}if(!_isArray(data)){data=[data]}if(data.length==0){return async.setImmediate(function(){if(q.drain){q.drain()}})}_each(data,function(task){var item={data:task,priority:priority,callback:typeof callback==="function"?callback:null};q.tasks.splice(_binarySearch(q.tasks,item,_compareTasks)+1,0,item);if(q.saturated&&q.tasks.length===q.concurrency){q.saturated()}async.setImmediate(q.process)})}var q=async.queue(worker,concurrency);q.push=function(data,priority,callback){_insert(q,data,priority,callback)};delete q.unshift;return q};async.cargo=function(worker,payload){var working=false,tasks=[];var cargo={tasks:tasks,payload:payload,saturated:null,empty:null,drain:null,drained:true,push:function(data,callback){if(!_isArray(data)){data=[data]}_each(data,function(task){tasks.push({data:task,callback:typeof callback==="function"?callback:null});cargo.drained=false;if(cargo.saturated&&tasks.length===payload){cargo.saturated()}});async.setImmediate(cargo.process)},process:function process(){if(working)return;if(tasks.length===0){if(cargo.drain&&!cargo.drained)cargo.drain();cargo.drained=true;return}var ts=typeof payload==="number"?tasks.splice(0,payload):tasks.splice(0,tasks.length);var ds=_map(ts,function(task){return task.data});if(cargo.empty)cargo.empty();working=true;worker(ds,function(){working=false;var args=arguments;_each(ts,function(data){if(data.callback){data.callback.apply(null,args)}});process()})},length:function(){return tasks.length},running:function(){return working}};return cargo};var _console_fn=function(name){return function(fn){var args=Array.prototype.slice.call(arguments,1);fn.apply(null,args.concat([function(err){var args=Array.prototype.slice.call(arguments,1);if(typeof console!=="undefined"){if(err){if(console.error){console.error(err)}}else if(console[name]){_each(args,function(x){console[name](x)})}}}]))}};async.log=_console_fn("log");async.dir=_console_fn("dir");async.memoize=function(fn,hasher){var memo={};var queues={};hasher=hasher||function(x){return x};var memoized=function(){var args=Array.prototype.slice.call(arguments);var callback=args.pop();var key=hasher.apply(null,args);if(key in memo){async.nextTick(function(){callback.apply(null,memo[key])})}else if(key in queues){queues[key].push(callback)}else{queues[key]=[callback];fn.apply(null,args.concat([function(){memo[key]=arguments;var q=queues[key];delete queues[key];for(var i=0,l=q.length;i<l;i++){q[i].apply(null,arguments)}}]))}};memoized.memo=memo;memoized.unmemoized=fn;return memoized};async.unmemoize=function(fn){return function(){return(fn.unmemoized||fn).apply(null,arguments)}};async.times=function(count,iterator,callback){var counter=[];for(var i=0;i<count;i++){counter.push(i)}return async.map(counter,iterator,callback)};async.timesSeries=function(count,iterator,callback){var counter=[];for(var i=0;i<count;i++){counter.push(i)}return async.mapSeries(counter,iterator,callback)};async.seq=function(){var fns=arguments;return function(){var that=this;var args=Array.prototype.slice.call(arguments);var callback=args.pop();async.reduce(fns,args,function(newargs,fn,cb){fn.apply(that,newargs.concat([function(){var err=arguments[0];var nextargs=Array.prototype.slice.call(arguments,1);cb(err,nextargs)}]))},function(err,results){callback.apply(that,[err].concat(results))})}};async.compose=function(){return async.seq.apply(null,Array.prototype.reverse.call(arguments))};var _applyEach=function(eachfn,fns){var go=function(){var that=this;var args=Array.prototype.slice.call(arguments);var callback=args.pop();return eachfn(fns,function(fn,cb){fn.apply(that,args.concat([cb]))},callback)};if(arguments.length>2){var args=Array.prototype.slice.call(arguments,2);return go.apply(this,args)}else{return go}};async.applyEach=doParallel(_applyEach);async.applyEachSeries=doSeries(_applyEach);async.forever=function(fn,callback){function next(err){if(err){if(callback){return callback(err)}throw err}fn(next)}next()};if(typeof module!=="undefined"&&module.exports){module.exports=async}else if(typeof define!=="undefined"&&define.amd){define([],function(){return async})}else{root.async=async}})()}).call(this,require("_process"))},{_process:33}],31:[function(require,module,exports){(function(root,factory){if(typeof define==="function"&&define.amd){define(["underscore","jquery","exports"],function(_,$,exports){root.Backbone=factory(root,exports,_,$)})}else if(typeof exports!=="undefined"){var _=require("underscore");factory(root,exports,_)}else{root.Backbone=factory(root,{},root._,root.jQuery||root.Zepto||root.ender||root.$)}})(this,function(root,Backbone,_,$){var previousBackbone=root.Backbone;var array=[];var push=array.push;var slice=array.slice;var splice=array.splice;Backbone.VERSION="1.1.2";Backbone.$=$;Backbone.noConflict=function(){root.Backbone=previousBackbone;return this};Backbone.emulateHTTP=false;Backbone.emulateJSON=false;var Events=Backbone.Events={on:function(name,callback,context){if(!eventsApi(this,"on",name,[callback,context])||!callback)return this;this._events||(this._events={});var events=this._events[name]||(this._events[name]=[]);events.push({callback:callback,context:context,ctx:context||this});return this},once:function(name,callback,context){if(!eventsApi(this,"once",name,[callback,context])||!callback)return this;var self=this;var once=_.once(function(){self.off(name,once);callback.apply(this,arguments)});once._callback=callback;return this.on(name,once,context)},off:function(name,callback,context){var retain,ev,events,names,i,l,j,k;if(!this._events||!eventsApi(this,"off",name,[callback,context]))return this;if(!name&&!callback&&!context){this._events=void 0;return this}names=name?[name]:_.keys(this._events);for(i=0,l=names.length;i<l;i++){name=names[i];if(events=this._events[name]){this._events[name]=retain=[];if(callback||context){for(j=0,k=events.length;j<k;j++){ev=events[j];if(callback&&callback!==ev.callback&&callback!==ev.callback._callback||context&&context!==ev.context){retain.push(ev)}}}if(!retain.length)delete this._events[name]}}return this},trigger:function(name){if(!this._events)return this;var args=slice.call(arguments,1);if(!eventsApi(this,"trigger",name,args))return this;var events=this._events[name];var allEvents=this._events.all;if(events)triggerEvents(events,args);if(allEvents)triggerEvents(allEvents,arguments);return this},stopListening:function(obj,name,callback){var listeningTo=this._listeningTo;if(!listeningTo)return this;var remove=!name&&!callback;if(!callback&&typeof name==="object")callback=this;if(obj)(listeningTo={})[obj._listenId]=obj;for(var id in listeningTo){obj=listeningTo[id];obj.off(name,callback,this);if(remove||_.isEmpty(obj._events))delete this._listeningTo[id]}return this}};var eventSplitter=/\s+/;var eventsApi=function(obj,action,name,rest){if(!name)return true;if(typeof name==="object"){for(var key in name){obj[action].apply(obj,[key,name[key]].concat(rest))}return false}if(eventSplitter.test(name)){var names=name.split(eventSplitter);for(var i=0,l=names.length;i<l;i++){obj[action].apply(obj,[names[i]].concat(rest))}return false}return true};var triggerEvents=function(events,args){var ev,i=-1,l=events.length,a1=args[0],a2=args[1],a3=args[2];switch(args.length){case 0:while(++i<l)(ev=events[i]).callback.call(ev.ctx);return;case 1:while(++i<l)(ev=events[i]).callback.call(ev.ctx,a1);return;case 2:while(++i<l)(ev=events[i]).callback.call(ev.ctx,a1,a2);return;case 3:while(++i<l)(ev=events[i]).callback.call(ev.ctx,a1,a2,a3);return;default:while(++i<l)(ev=events[i]).callback.apply(ev.ctx,args);return}};var listenMethods={listenTo:"on",listenToOnce:"once"};_.each(listenMethods,function(implementation,method){Events[method]=function(obj,name,callback){var listeningTo=this._listeningTo||(this._listeningTo={});var id=obj._listenId||(obj._listenId=_.uniqueId("l"));listeningTo[id]=obj;if(!callback&&typeof name==="object")callback=this;obj[implementation](name,callback,this);return this}});Events.bind=Events.on;Events.unbind=Events.off;_.extend(Backbone,Events);var Model=Backbone.Model=function(attributes,options){var attrs=attributes||{};options||(options={});this.cid=_.uniqueId("c");this.attributes={};if(options.collection)this.collection=options.collection;if(options.parse)attrs=this.parse(attrs,options)||{};attrs=_.defaults({},attrs,_.result(this,"defaults"));this.set(attrs,options);this.changed={};this.initialize.apply(this,arguments)};_.extend(Model.prototype,Events,{changed:null,validationError:null,idAttribute:"id",initialize:function(){},toJSON:function(options){return _.clone(this.attributes)},sync:function(){return Backbone.sync.apply(this,arguments)},get:function(attr){return this.attributes[attr]},escape:function(attr){return _.escape(this.get(attr))},has:function(attr){return this.get(attr)!=null},set:function(key,val,options){var attr,attrs,unset,changes,silent,changing,prev,current;if(key==null)return this;if(typeof key==="object"){attrs=key;options=val}else{(attrs={})[key]=val}options||(options={});if(!this._validate(attrs,options))return false;unset=options.unset;silent=options.silent;changes=[];changing=this._changing;this._changing=true;if(!changing){this._previousAttributes=_.clone(this.attributes);this.changed={}}current=this.attributes,prev=this._previousAttributes;if(this.idAttribute in attrs)this.id=attrs[this.idAttribute];for(attr in attrs){val=attrs[attr];if(!_.isEqual(current[attr],val))changes.push(attr);if(!_.isEqual(prev[attr],val)){this.changed[attr]=val}else{delete this.changed[attr]}unset?delete current[attr]:current[attr]=val}if(!silent){if(changes.length)this._pending=options;for(var i=0,l=changes.length;i<l;i++){this.trigger("change:"+changes[i],this,current[changes[i]],options)}}if(changing)return this;if(!silent){while(this._pending){options=this._pending;this._pending=false;this.trigger("change",this,options)}}this._pending=false;this._changing=false;return this},unset:function(attr,options){return this.set(attr,void 0,_.extend({},options,{unset:true}))},clear:function(options){var attrs={};for(var key in this.attributes)attrs[key]=void 0;return this.set(attrs,_.extend({},options,{unset:true}))},hasChanged:function(attr){if(attr==null)return!_.isEmpty(this.changed);return _.has(this.changed,attr)},changedAttributes:function(diff){if(!diff)return this.hasChanged()?_.clone(this.changed):false;var val,changed=false;var old=this._changing?this._previousAttributes:this.attributes;for(var attr in diff){if(_.isEqual(old[attr],val=diff[attr]))continue;(changed||(changed={}))[attr]=val}return changed},previous:function(attr){if(attr==null||!this._previousAttributes)return null;return this._previousAttributes[attr]},previousAttributes:function(){return _.clone(this._previousAttributes)},fetch:function(options){options=options?_.clone(options):{};if(options.parse===void 0)options.parse=true;var model=this;var success=options.success;options.success=function(resp){if(!model.set(model.parse(resp,options),options))return false;if(success)success(model,resp,options);model.trigger("sync",model,resp,options)};wrapError(this,options);return this.sync("read",this,options)},save:function(key,val,options){var attrs,method,xhr,attributes=this.attributes;if(key==null||typeof key==="object"){attrs=key;options=val}else{(attrs={})[key]=val}options=_.extend({validate:true},options);if(attrs&&!options.wait){if(!this.set(attrs,options))return false}else{if(!this._validate(attrs,options))return false}if(attrs&&options.wait){this.attributes=_.extend({},attributes,attrs)}if(options.parse===void 0)options.parse=true;var model=this;var success=options.success;options.success=function(resp){model.attributes=attributes;var serverAttrs=model.parse(resp,options);if(options.wait)serverAttrs=_.extend(attrs||{},serverAttrs);if(_.isObject(serverAttrs)&&!model.set(serverAttrs,options)){return false}if(success)success(model,resp,options);model.trigger("sync",model,resp,options)};wrapError(this,options);method=this.isNew()?"create":options.patch?"patch":"update";if(method==="patch")options.attrs=attrs;xhr=this.sync(method,this,options);if(attrs&&options.wait)this.attributes=attributes;return xhr},destroy:function(options){options=options?_.clone(options):{};var model=this;var success=options.success;var destroy=function(){model.trigger("destroy",model,model.collection,options)};options.success=function(resp){if(options.wait||model.isNew())destroy();if(success)success(model,resp,options);if(!model.isNew())model.trigger("sync",model,resp,options)};if(this.isNew()){options.success();return false}wrapError(this,options);var xhr=this.sync("delete",this,options);if(!options.wait)destroy();return xhr},url:function(){var base=_.result(this,"urlRoot")||_.result(this.collection,"url")||urlError();if(this.isNew())return base;return base.replace(/([^\/])$/,"$1/")+encodeURIComponent(this.id)},parse:function(resp,options){return resp},clone:function(){return new this.constructor(this.attributes)},isNew:function(){return!this.has(this.idAttribute)},isValid:function(options){return this._validate({},_.extend(options||{},{validate:true}))},_validate:function(attrs,options){if(!options.validate||!this.validate)return true;attrs=_.extend({},this.attributes,attrs);var error=this.validationError=this.validate(attrs,options)||null;if(!error)return true;this.trigger("invalid",this,error,_.extend(options,{validationError:error}));return false}});var modelMethods=["keys","values","pairs","invert","pick","omit"];_.each(modelMethods,function(method){Model.prototype[method]=function(){var args=slice.call(arguments);args.unshift(this.attributes);return _[method].apply(_,args)}});var Collection=Backbone.Collection=function(models,options){options||(options={});if(options.model)this.model=options.model;if(options.comparator!==void 0)this.comparator=options.comparator;this._reset();this.initialize.apply(this,arguments);if(models)this.reset(models,_.extend({silent:true},options))};var setOptions={add:true,remove:true,merge:true};var addOptions={add:true,remove:false};_.extend(Collection.prototype,Events,{model:Model,initialize:function(){},toJSON:function(options){return this.map(function(model){return model.toJSON(options)})},sync:function(){return Backbone.sync.apply(this,arguments)},add:function(models,options){return this.set(models,_.extend({merge:false},options,addOptions))},remove:function(models,options){var singular=!_.isArray(models);models=singular?[models]:_.clone(models);options||(options={});var i,l,index,model;for(i=0,l=models.length;i<l;i++){model=models[i]=this.get(models[i]);if(!model)continue;delete this._byId[model.id];delete this._byId[model.cid];index=this.indexOf(model);this.models.splice(index,1);this.length--;if(!options.silent){options.index=index;model.trigger("remove",model,this,options)}this._removeReference(model,options)}return singular?models[0]:models},set:function(models,options){options=_.defaults({},options,setOptions);if(options.parse)models=this.parse(models,options);var singular=!_.isArray(models);models=singular?models?[models]:[]:_.clone(models);var i,l,id,model,attrs,existing,sort;var at=options.at;var targetModel=this.model;var sortable=this.comparator&&at==null&&options.sort!==false;var sortAttr=_.isString(this.comparator)?this.comparator:null;var toAdd=[],toRemove=[],modelMap={};var add=options.add,merge=options.merge,remove=options.remove;var order=!sortable&&add&&remove?[]:false;for(i=0,l=models.length;i<l;i++){attrs=models[i]||{};if(attrs instanceof Model){id=model=attrs}else{id=attrs[targetModel.prototype.idAttribute||"id"]}if(existing=this.get(id)){if(remove)modelMap[existing.cid]=true;if(merge){attrs=attrs===model?model.attributes:attrs;if(options.parse)attrs=existing.parse(attrs,options);existing.set(attrs,options);if(sortable&&!sort&&existing.hasChanged(sortAttr))sort=true}models[i]=existing}else if(add){model=models[i]=this._prepareModel(attrs,options);if(!model)continue;toAdd.push(model);this._addReference(model,options)}model=existing||model;if(order&&(model.isNew()||!modelMap[model.id]))order.push(model);modelMap[model.id]=true}if(remove){for(i=0,l=this.length;i<l;++i){if(!modelMap[(model=this.models[i]).cid])toRemove.push(model)}if(toRemove.length)this.remove(toRemove,options)}if(toAdd.length||order&&order.length){if(sortable)sort=true;this.length+=toAdd.length;if(at!=null){for(i=0,l=toAdd.length;i<l;i++){this.models.splice(at+i,0,toAdd[i])}}else{if(order)this.models.length=0;var orderedModels=order||toAdd;for(i=0,l=orderedModels.length;i<l;i++){this.models.push(orderedModels[i])}}}if(sort)this.sort({silent:true});if(!options.silent){for(i=0,l=toAdd.length;i<l;i++){(model=toAdd[i]).trigger("add",model,this,options)}if(sort||order&&order.length)this.trigger("sort",this,options)}return singular?models[0]:models},reset:function(models,options){options||(options={});for(var i=0,l=this.models.length;i<l;i++){this._removeReference(this.models[i],options)}options.previousModels=this.models;this._reset();models=this.add(models,_.extend({silent:true},options));if(!options.silent)this.trigger("reset",this,options);return models},push:function(model,options){return this.add(model,_.extend({at:this.length},options))},pop:function(options){var model=this.at(this.length-1);this.remove(model,options);return model},unshift:function(model,options){return this.add(model,_.extend({at:0},options))},shift:function(options){var model=this.at(0);this.remove(model,options);return model},slice:function(){return slice.apply(this.models,arguments)},get:function(obj){if(obj==null)return void 0;return this._byId[obj]||this._byId[obj.id]||this._byId[obj.cid]},at:function(index){return this.models[index]},where:function(attrs,first){if(_.isEmpty(attrs))return first?void 0:[];return this[first?"find":"filter"](function(model){
for(var key in attrs){if(attrs[key]!==model.get(key))return false}return true})},findWhere:function(attrs){return this.where(attrs,true)},sort:function(options){if(!this.comparator)throw new Error("Cannot sort a set without a comparator");options||(options={});if(_.isString(this.comparator)||this.comparator.length===1){this.models=this.sortBy(this.comparator,this)}else{this.models.sort(_.bind(this.comparator,this))}if(!options.silent)this.trigger("sort",this,options);return this},pluck:function(attr){return _.invoke(this.models,"get",attr)},fetch:function(options){options=options?_.clone(options):{};if(options.parse===void 0)options.parse=true;var success=options.success;var collection=this;options.success=function(resp){var method=options.reset?"reset":"set";collection[method](resp,options);if(success)success(collection,resp,options);collection.trigger("sync",collection,resp,options)};wrapError(this,options);return this.sync("read",this,options)},create:function(model,options){options=options?_.clone(options):{};if(!(model=this._prepareModel(model,options)))return false;if(!options.wait)this.add(model,options);var collection=this;var success=options.success;options.success=function(model,resp){if(options.wait)collection.add(model,options);if(success)success(model,resp,options)};model.save(null,options);return model},parse:function(resp,options){return resp},clone:function(){return new this.constructor(this.models)},_reset:function(){this.length=0;this.models=[];this._byId={}},_prepareModel:function(attrs,options){if(attrs instanceof Model)return attrs;options=options?_.clone(options):{};options.collection=this;var model=new this.model(attrs,options);if(!model.validationError)return model;this.trigger("invalid",this,model.validationError,options);return false},_addReference:function(model,options){this._byId[model.cid]=model;if(model.id!=null)this._byId[model.id]=model;if(!model.collection)model.collection=this;model.on("all",this._onModelEvent,this)},_removeReference:function(model,options){if(this===model.collection)delete model.collection;model.off("all",this._onModelEvent,this)},_onModelEvent:function(event,model,collection,options){if((event==="add"||event==="remove")&&collection!==this)return;if(event==="destroy")this.remove(model,options);if(model&&event==="change:"+model.idAttribute){delete this._byId[model.previous(model.idAttribute)];if(model.id!=null)this._byId[model.id]=model}this.trigger.apply(this,arguments)}});var methods=["forEach","each","map","collect","reduce","foldl","inject","reduceRight","foldr","find","detect","filter","select","reject","every","all","some","any","include","contains","invoke","max","min","toArray","size","first","head","take","initial","rest","tail","drop","last","without","difference","indexOf","shuffle","lastIndexOf","isEmpty","chain","sample"];_.each(methods,function(method){Collection.prototype[method]=function(){var args=slice.call(arguments);args.unshift(this.models);return _[method].apply(_,args)}});var attributeMethods=["groupBy","countBy","sortBy","indexBy"];_.each(attributeMethods,function(method){Collection.prototype[method]=function(value,context){var iterator=_.isFunction(value)?value:function(model){return model.get(value)};return _[method](this.models,iterator,context)}});var View=Backbone.View=function(options){this.cid=_.uniqueId("view");options||(options={});_.extend(this,_.pick(options,viewOptions));this._ensureElement();this.initialize.apply(this,arguments);this.delegateEvents()};var delegateEventSplitter=/^(\S+)\s*(.*)$/;var viewOptions=["model","collection","el","id","attributes","className","tagName","events"];_.extend(View.prototype,Events,{tagName:"div",$:function(selector){return this.$el.find(selector)},initialize:function(){},render:function(){return this},remove:function(){this.$el.remove();this.stopListening();return this},setElement:function(element,delegate){if(this.$el)this.undelegateEvents();this.$el=element instanceof Backbone.$?element:Backbone.$(element);this.el=this.$el[0];if(delegate!==false)this.delegateEvents();return this},delegateEvents:function(events){if(!(events||(events=_.result(this,"events"))))return this;this.undelegateEvents();for(var key in events){var method=events[key];if(!_.isFunction(method))method=this[events[key]];if(!method)continue;var match=key.match(delegateEventSplitter);var eventName=match[1],selector=match[2];method=_.bind(method,this);eventName+=".delegateEvents"+this.cid;if(selector===""){this.$el.on(eventName,method)}else{this.$el.on(eventName,selector,method)}}return this},undelegateEvents:function(){this.$el.off(".delegateEvents"+this.cid);return this},_ensureElement:function(){if(!this.el){var attrs=_.extend({},_.result(this,"attributes"));if(this.id)attrs.id=_.result(this,"id");if(this.className)attrs["class"]=_.result(this,"className");var $el=Backbone.$("<"+_.result(this,"tagName")+">").attr(attrs);this.setElement($el,false)}else{this.setElement(_.result(this,"el"),false)}}});Backbone.sync=function(method,model,options){var type=methodMap[method];_.defaults(options||(options={}),{emulateHTTP:Backbone.emulateHTTP,emulateJSON:Backbone.emulateJSON});var params={type:type,dataType:"json"};if(!options.url){params.url=_.result(model,"url")||urlError()}if(options.data==null&&model&&(method==="create"||method==="update"||method==="patch")){params.contentType="application/json";params.data=JSON.stringify(options.attrs||model.toJSON(options))}if(options.emulateJSON){params.contentType="application/x-www-form-urlencoded";params.data=params.data?{model:params.data}:{}}if(options.emulateHTTP&&(type==="PUT"||type==="DELETE"||type==="PATCH")){params.type="POST";if(options.emulateJSON)params.data._method=type;var beforeSend=options.beforeSend;options.beforeSend=function(xhr){xhr.setRequestHeader("X-HTTP-Method-Override",type);if(beforeSend)return beforeSend.apply(this,arguments)}}if(params.type!=="GET"&&!options.emulateJSON){params.processData=false}if(params.type==="PATCH"&&noXhrPatch){params.xhr=function(){return new ActiveXObject("Microsoft.XMLHTTP")}}var xhr=options.xhr=Backbone.ajax(_.extend(params,options));model.trigger("request",model,xhr,options);return xhr};var noXhrPatch=typeof window!=="undefined"&&!!window.ActiveXObject&&!(window.XMLHttpRequest&&(new XMLHttpRequest).dispatchEvent);var methodMap={create:"POST",update:"PUT",patch:"PATCH","delete":"DELETE",read:"GET"};Backbone.ajax=function(){return Backbone.$.ajax.apply(Backbone.$,arguments)};var Router=Backbone.Router=function(options){options||(options={});if(options.routes)this.routes=options.routes;this._bindRoutes();this.initialize.apply(this,arguments)};var optionalParam=/\((.*?)\)/g;var namedParam=/(\(\?)?:\w+/g;var splatParam=/\*\w+/g;var escapeRegExp=/[\-{}\[\]+?.,\\\^$|#\s]/g;_.extend(Router.prototype,Events,{initialize:function(){},route:function(route,name,callback){if(!_.isRegExp(route))route=this._routeToRegExp(route);if(_.isFunction(name)){callback=name;name=""}if(!callback)callback=this[name];var router=this;Backbone.history.route(route,function(fragment){var args=router._extractParameters(route,fragment);router.execute(callback,args);router.trigger.apply(router,["route:"+name].concat(args));router.trigger("route",name,args);Backbone.history.trigger("route",router,name,args)});return this},execute:function(callback,args){if(callback)callback.apply(this,args)},navigate:function(fragment,options){Backbone.history.navigate(fragment,options);return this},_bindRoutes:function(){if(!this.routes)return;this.routes=_.result(this,"routes");var route,routes=_.keys(this.routes);while((route=routes.pop())!=null){this.route(route,this.routes[route])}},_routeToRegExp:function(route){route=route.replace(escapeRegExp,"\\$&").replace(optionalParam,"(?:$1)?").replace(namedParam,function(match,optional){return optional?match:"([^/?]+)"}).replace(splatParam,"([^?]*?)");return new RegExp("^"+route+"(?:\\?([\\s\\S]*))?$")},_extractParameters:function(route,fragment){var params=route.exec(fragment).slice(1);return _.map(params,function(param,i){if(i===params.length-1)return param||null;return param?decodeURIComponent(param):null})}});var History=Backbone.History=function(){this.handlers=[];_.bindAll(this,"checkUrl");if(typeof window!=="undefined"){this.location=window.location;this.history=window.history}};var routeStripper=/^[#\/]|\s+$/g;var rootStripper=/^\/+|\/+$/g;var isExplorer=/msie [\w.]+/;var trailingSlash=/\/$/;var pathStripper=/#.*$/;History.started=false;_.extend(History.prototype,Events,{interval:50,atRoot:function(){return this.location.pathname.replace(/[^\/]$/,"$&/")===this.root},getHash:function(window){var match=(window||this).location.href.match(/#(.*)$/);return match?match[1]:""},getFragment:function(fragment,forcePushState){if(fragment==null){if(this._hasPushState||!this._wantsHashChange||forcePushState){fragment=decodeURI(this.location.pathname+this.location.search);var root=this.root.replace(trailingSlash,"");if(!fragment.indexOf(root))fragment=fragment.slice(root.length)}else{fragment=this.getHash()}}return fragment.replace(routeStripper,"")},start:function(options){if(History.started)throw new Error("Backbone.history has already been started");History.started=true;this.options=_.extend({root:"/"},this.options,options);this.root=this.options.root;this._wantsHashChange=this.options.hashChange!==false;this._wantsPushState=!!this.options.pushState;this._hasPushState=!!(this.options.pushState&&this.history&&this.history.pushState);var fragment=this.getFragment();var docMode=document.documentMode;var oldIE=isExplorer.exec(navigator.userAgent.toLowerCase())&&(!docMode||docMode<=7);this.root=("/"+this.root+"/").replace(rootStripper,"/");if(oldIE&&this._wantsHashChange){var frame=Backbone.$('<iframe src="javascript:0" tabindex="-1">');this.iframe=frame.hide().appendTo("body")[0].contentWindow;this.navigate(fragment)}if(this._hasPushState){Backbone.$(window).on("popstate",this.checkUrl)}else if(this._wantsHashChange&&"onhashchange"in window&&!oldIE){Backbone.$(window).on("hashchange",this.checkUrl)}else if(this._wantsHashChange){this._checkUrlInterval=setInterval(this.checkUrl,this.interval)}this.fragment=fragment;var loc=this.location;if(this._wantsHashChange&&this._wantsPushState){if(!this._hasPushState&&!this.atRoot()){this.fragment=this.getFragment(null,true);this.location.replace(this.root+"#"+this.fragment);return true}else if(this._hasPushState&&this.atRoot()&&loc.hash){this.fragment=this.getHash().replace(routeStripper,"");this.history.replaceState({},document.title,this.root+this.fragment)}}if(!this.options.silent)return this.loadUrl()},stop:function(){Backbone.$(window).off("popstate",this.checkUrl).off("hashchange",this.checkUrl);if(this._checkUrlInterval)clearInterval(this._checkUrlInterval);History.started=false},route:function(route,callback){this.handlers.unshift({route:route,callback:callback})},checkUrl:function(e){var current=this.getFragment();if(current===this.fragment&&this.iframe){current=this.getFragment(this.getHash(this.iframe))}if(current===this.fragment)return false;if(this.iframe)this.navigate(current);this.loadUrl()},loadUrl:function(fragment){fragment=this.fragment=this.getFragment(fragment);return _.any(this.handlers,function(handler){if(handler.route.test(fragment)){handler.callback(fragment);return true}})},navigate:function(fragment,options){if(!History.started)return false;if(!options||options===true)options={trigger:!!options};var url=this.root+(fragment=this.getFragment(fragment||""));fragment=fragment.replace(pathStripper,"");if(this.fragment===fragment)return;this.fragment=fragment;if(fragment===""&&url!=="/")url=url.slice(0,-1);if(this._hasPushState){this.history[options.replace?"replaceState":"pushState"]({},document.title,url)}else if(this._wantsHashChange){this._updateHash(this.location,fragment,options.replace);if(this.iframe&&fragment!==this.getFragment(this.getHash(this.iframe))){if(!options.replace)this.iframe.document.open().close();this._updateHash(this.iframe.location,fragment,options.replace)}}else{return this.location.assign(url)}if(options.trigger)return this.loadUrl(fragment)},_updateHash:function(location,fragment,replace){if(replace){var href=location.href.replace(/(javascript:|#).*$/,"");location.replace(href+"#"+fragment)}else{location.hash="#"+fragment}}});Backbone.history=new History;var extend=function(protoProps,staticProps){var parent=this;var child;if(protoProps&&_.has(protoProps,"constructor")){child=protoProps.constructor}else{child=function(){return parent.apply(this,arguments)}}_.extend(child,parent,staticProps);var Surrogate=function(){this.constructor=child};Surrogate.prototype=parent.prototype;child.prototype=new Surrogate;if(protoProps)_.extend(child.prototype,protoProps);child.__super__=parent.prototype;return child};Model.extend=Collection.extend=Router.extend=View.extend=History.extend=extend;var urlError=function(){throw new Error('A "url" property or function must be specified')};var wrapError=function(model,options){var error=options.error;options.error=function(resp){if(error)error(model,resp,options);model.trigger("error",model,resp,options)}};return Backbone})},{underscore:202}],32:[function(require,module,exports){function EventEmitter(){this._events=this._events||{};this._maxListeners=this._maxListeners||undefined}module.exports=EventEmitter;EventEmitter.EventEmitter=EventEmitter;EventEmitter.prototype._events=undefined;EventEmitter.prototype._maxListeners=undefined;EventEmitter.defaultMaxListeners=10;EventEmitter.prototype.setMaxListeners=function(n){if(!isNumber(n)||n<0||isNaN(n))throw TypeError("n must be a positive number");this._maxListeners=n;return this};EventEmitter.prototype.emit=function(type){var er,handler,len,args,i,listeners;if(!this._events)this._events={};if(type==="error"){if(!this._events.error||isObject(this._events.error)&&!this._events.error.length){er=arguments[1];if(er instanceof Error){throw er}throw TypeError('Uncaught, unspecified "error" event.')}}handler=this._events[type];if(isUndefined(handler))return false;if(isFunction(handler)){switch(arguments.length){case 1:handler.call(this);break;case 2:handler.call(this,arguments[1]);break;case 3:handler.call(this,arguments[1],arguments[2]);break;default:len=arguments.length;args=new Array(len-1);for(i=1;i<len;i++)args[i-1]=arguments[i];handler.apply(this,args)}}else if(isObject(handler)){len=arguments.length;args=new Array(len-1);for(i=1;i<len;i++)args[i-1]=arguments[i];listeners=handler.slice();len=listeners.length;for(i=0;i<len;i++)listeners[i].apply(this,args)}return true};EventEmitter.prototype.addListener=function(type,listener){var m;if(!isFunction(listener))throw TypeError("listener must be a function");if(!this._events)this._events={};if(this._events.newListener)this.emit("newListener",type,isFunction(listener.listener)?listener.listener:listener);if(!this._events[type])this._events[type]=listener;else if(isObject(this._events[type]))this._events[type].push(listener);else this._events[type]=[this._events[type],listener];if(isObject(this._events[type])&&!this._events[type].warned){var m;if(!isUndefined(this._maxListeners)){m=this._maxListeners}else{m=EventEmitter.defaultMaxListeners}if(m&&m>0&&this._events[type].length>m){this._events[type].warned=true;console.error("(node) warning: possible EventEmitter memory "+"leak detected. %d listeners added. "+"Use emitter.setMaxListeners() to increase limit.",this._events[type].length);if(typeof console.trace==="function"){console.trace()}}}return this};EventEmitter.prototype.on=EventEmitter.prototype.addListener;EventEmitter.prototype.once=function(type,listener){if(!isFunction(listener))throw TypeError("listener must be a function");var fired=false;function g(){this.removeListener(type,g);if(!fired){fired=true;listener.apply(this,arguments)}}g.listener=listener;this.on(type,g);return this};EventEmitter.prototype.removeListener=function(type,listener){var list,position,length,i;if(!isFunction(listener))throw TypeError("listener must be a function");if(!this._events||!this._events[type])return this;list=this._events[type];length=list.length;position=-1;if(list===listener||isFunction(list.listener)&&list.listener===listener){delete this._events[type];if(this._events.removeListener)this.emit("removeListener",type,listener)}else if(isObject(list)){for(i=length;i-->0;){if(list[i]===listener||list[i].listener&&list[i].listener===listener){position=i;break}}if(position<0)return this;if(list.length===1){list.length=0;delete this._events[type]}else{list.splice(position,1)}if(this._events.removeListener)this.emit("removeListener",type,listener)}return this};EventEmitter.prototype.removeAllListeners=function(type){var key,listeners;if(!this._events)return this;if(!this._events.removeListener){if(arguments.length===0)this._events={};else if(this._events[type])delete this._events[type];return this}if(arguments.length===0){for(key in this._events){if(key==="removeListener")continue;this.removeAllListeners(key)}this.removeAllListeners("removeListener");this._events={};return this}listeners=this._events[type];if(isFunction(listeners)){this.removeListener(type,listeners)}else{while(listeners.length)this.removeListener(type,listeners[listeners.length-1])}delete this._events[type];return this};EventEmitter.prototype.listeners=function(type){var ret;if(!this._events||!this._events[type])ret=[];else if(isFunction(this._events[type]))ret=[this._events[type]];else ret=this._events[type].slice();return ret};EventEmitter.listenerCount=function(emitter,type){var ret;if(!emitter._events||!emitter._events[type])ret=0;else if(isFunction(emitter._events[type]))ret=1;else ret=emitter._events[type].length;return ret};function isFunction(arg){return typeof arg==="function"}function isNumber(arg){return typeof arg==="number"}function isObject(arg){return typeof arg==="object"&&arg!==null}function isUndefined(arg){return arg===void 0}},{}],33:[function(require,module,exports){var process=module.exports={};var queue=[];var draining=false;function drainQueue(){if(draining){return}draining=true;var currentQueue;var len=queue.length;while(len){currentQueue=queue;queue=[];var i=-1;while(++i<len){currentQueue[i]()}len=queue.length}draining=false}process.nextTick=function(fun){queue.push(fun);if(!draining){setTimeout(drainQueue,0)}};process.title="browser";process.browser=true;process.env={};process.argv=[];process.version="";process.versions={};function noop(){}process.on=noop;process.addListener=noop;process.once=noop;process.off=noop;process.removeListener=noop;process.removeAllListeners=noop;process.emit=noop;process.binding=function(name){throw new Error("process.binding is not supported")};process.cwd=function(){return"/"};process.chdir=function(dir){throw new Error("process.chdir is not supported")};process.umask=function(){return 0}},{}],34:[function(require,module,exports){(function(global,factory){if(typeof module==="object"&&typeof module.exports==="object"){module.exports=global.document?factory(global,true):function(w){if(!w.document){throw new Error("jQuery requires a window with a document")}return factory(w)}}else{factory(global)}})(typeof window!=="undefined"?window:this,function(window,noGlobal){var arr=[];var slice=arr.slice;var concat=arr.concat;var push=arr.push;var indexOf=arr.indexOf;var class2type={};var toString=class2type.toString;var hasOwn=class2type.hasOwnProperty;var support={};var document=window.document,version="2.1.3",jQuery=function(selector,context){return new jQuery.fn.init(selector,context)},rtrim=/^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g,rmsPrefix=/^-ms-/,rdashAlpha=/-([\da-z])/gi,fcamelCase=function(all,letter){return letter.toUpperCase()};jQuery.fn=jQuery.prototype={jquery:version,constructor:jQuery,selector:"",length:0,toArray:function(){return slice.call(this)},get:function(num){return num!=null?num<0?this[num+this.length]:this[num]:slice.call(this)},pushStack:function(elems){var ret=jQuery.merge(this.constructor(),elems);ret.prevObject=this;ret.context=this.context;return ret},each:function(callback,args){return jQuery.each(this,callback,args)},map:function(callback){return this.pushStack(jQuery.map(this,function(elem,i){return callback.call(elem,i,elem)}))},slice:function(){return this.pushStack(slice.apply(this,arguments))},first:function(){return this.eq(0)},last:function(){return this.eq(-1)},eq:function(i){var len=this.length,j=+i+(i<0?len:0);return this.pushStack(j>=0&&j<len?[this[j]]:[])},end:function(){return this.prevObject||this.constructor(null)},push:push,sort:arr.sort,splice:arr.splice};jQuery.extend=jQuery.fn.extend=function(){var options,name,src,copy,copyIsArray,clone,target=arguments[0]||{},i=1,length=arguments.length,deep=false;if(typeof target==="boolean"){deep=target;target=arguments[i]||{};i++}if(typeof target!=="object"&&!jQuery.isFunction(target)){target={}}if(i===length){target=this;i--}for(;i<length;i++){if((options=arguments[i])!=null){for(name in options){src=target[name];copy=options[name];if(target===copy){continue}if(deep&&copy&&(jQuery.isPlainObject(copy)||(copyIsArray=jQuery.isArray(copy)))){if(copyIsArray){copyIsArray=false;clone=src&&jQuery.isArray(src)?src:[]}else{clone=src&&jQuery.isPlainObject(src)?src:{}}target[name]=jQuery.extend(deep,clone,copy)}else if(copy!==undefined){target[name]=copy}}}}return target};jQuery.extend({expando:"jQuery"+(version+Math.random()).replace(/\D/g,""),isReady:true,error:function(msg){throw new Error(msg)},noop:function(){},isFunction:function(obj){return jQuery.type(obj)==="function"},isArray:Array.isArray,isWindow:function(obj){return obj!=null&&obj===obj.window},isNumeric:function(obj){return!jQuery.isArray(obj)&&obj-parseFloat(obj)+1>=0},isPlainObject:function(obj){if(jQuery.type(obj)!=="object"||obj.nodeType||jQuery.isWindow(obj)){return false}if(obj.constructor&&!hasOwn.call(obj.constructor.prototype,"isPrototypeOf")){return false}return true},isEmptyObject:function(obj){var name;for(name in obj){return false}return true},type:function(obj){if(obj==null){return obj+""}return typeof obj==="object"||typeof obj==="function"?class2type[toString.call(obj)]||"object":typeof obj},globalEval:function(code){var script,indirect=eval;code=jQuery.trim(code);if(code){if(code.indexOf("use strict")===1){script=document.createElement("script");script.text=code;document.head.appendChild(script).parentNode.removeChild(script)}else{indirect(code)}}},camelCase:function(string){return string.replace(rmsPrefix,"ms-").replace(rdashAlpha,fcamelCase)},nodeName:function(elem,name){return elem.nodeName&&elem.nodeName.toLowerCase()===name.toLowerCase()},each:function(obj,callback,args){var value,i=0,length=obj.length,isArray=isArraylike(obj);if(args){if(isArray){for(;i<length;i++){value=callback.apply(obj[i],args);if(value===false){break}}}else{for(i in obj){value=callback.apply(obj[i],args);if(value===false){break}}}}else{if(isArray){for(;i<length;i++){value=callback.call(obj[i],i,obj[i]);if(value===false){break}}}else{for(i in obj){value=callback.call(obj[i],i,obj[i]);if(value===false){break}}}}return obj},trim:function(text){return text==null?"":(text+"").replace(rtrim,"")},makeArray:function(arr,results){var ret=results||[];if(arr!=null){if(isArraylike(Object(arr))){jQuery.merge(ret,typeof arr==="string"?[arr]:arr)}else{push.call(ret,arr)}}return ret},inArray:function(elem,arr,i){return arr==null?-1:indexOf.call(arr,elem,i)},merge:function(first,second){var len=+second.length,j=0,i=first.length;for(;j<len;j++){first[i++]=second[j]}first.length=i;return first},grep:function(elems,callback,invert){var callbackInverse,matches=[],i=0,length=elems.length,callbackExpect=!invert;for(;i<length;i++){callbackInverse=!callback(elems[i],i);if(callbackInverse!==callbackExpect){matches.push(elems[i])}}return matches},map:function(elems,callback,arg){var value,i=0,length=elems.length,isArray=isArraylike(elems),ret=[];if(isArray){for(;i<length;i++){value=callback(elems[i],i,arg);if(value!=null){ret.push(value)}}}else{for(i in elems){value=callback(elems[i],i,arg);if(value!=null){ret.push(value)}}}return concat.apply([],ret)},guid:1,proxy:function(fn,context){var tmp,args,proxy;if(typeof context==="string"){tmp=fn[context];context=fn;fn=tmp}if(!jQuery.isFunction(fn)){return undefined}args=slice.call(arguments,2);proxy=function(){return fn.apply(context||this,args.concat(slice.call(arguments)))};proxy.guid=fn.guid=fn.guid||jQuery.guid++;return proxy},now:Date.now,support:support});jQuery.each("Boolean Number String Function Array Date RegExp Object Error".split(" "),function(i,name){class2type["[object "+name+"]"]=name.toLowerCase()});function isArraylike(obj){var length=obj.length,type=jQuery.type(obj);if(type==="function"||jQuery.isWindow(obj)){return false}if(obj.nodeType===1&&length){return true}return type==="array"||length===0||typeof length==="number"&&length>0&&length-1 in obj}var Sizzle=function(window){var i,support,Expr,getText,isXML,tokenize,compile,select,outermostContext,sortInput,hasDuplicate,setDocument,document,docElem,documentIsHTML,rbuggyQSA,rbuggyMatches,matches,contains,expando="sizzle"+1*new Date,preferredDoc=window.document,dirruns=0,done=0,classCache=createCache(),tokenCache=createCache(),compilerCache=createCache(),sortOrder=function(a,b){if(a===b){hasDuplicate=true}return 0},MAX_NEGATIVE=1<<31,hasOwn={}.hasOwnProperty,arr=[],pop=arr.pop,push_native=arr.push,push=arr.push,slice=arr.slice,indexOf=function(list,elem){var i=0,len=list.length;for(;i<len;i++){if(list[i]===elem){return i}}return-1},booleans="checked|selected|async|autofocus|autoplay|controls|defer|disabled|hidden|ismap|loop|multiple|open|readonly|required|scoped",whitespace="[\\x20\\t\\r\\n\\f]",characterEncoding="(?:\\\\.|[\\w-]|[^\\x00-\\xa0])+",identifier=characterEncoding.replace("w","w#"),attributes="\\["+whitespace+"*("+characterEncoding+")(?:"+whitespace+"*([*^$|!~]?=)"+whitespace+"*(?:'((?:\\\\.|[^\\\\'])*)'|\"((?:\\\\.|[^\\\\\"])*)\"|("+identifier+"))|)"+whitespace+"*\\]",pseudos=":("+characterEncoding+")(?:\\(("+"('((?:\\\\.|[^\\\\'])*)'|\"((?:\\\\.|[^\\\\\"])*)\")|"+"((?:\\\\.|[^\\\\()[\\]]|"+attributes+")*)|"+".*"+")\\)|)",rwhitespace=new RegExp(whitespace+"+","g"),rtrim=new RegExp("^"+whitespace+"+|((?:^|[^\\\\])(?:\\\\.)*)"+whitespace+"+$","g"),rcomma=new RegExp("^"+whitespace+"*,"+whitespace+"*"),rcombinators=new RegExp("^"+whitespace+"*([>+~]|"+whitespace+")"+whitespace+"*"),rattributeQuotes=new RegExp("="+whitespace+"*([^\\]'\"]*?)"+whitespace+"*\\]","g"),rpseudo=new RegExp(pseudos),ridentifier=new RegExp("^"+identifier+"$"),matchExpr={ID:new RegExp("^#("+characterEncoding+")"),CLASS:new RegExp("^\\.("+characterEncoding+")"),TAG:new RegExp("^("+characterEncoding.replace("w","w*")+")"),ATTR:new RegExp("^"+attributes),PSEUDO:new RegExp("^"+pseudos),CHILD:new RegExp("^:(only|first|last|nth|nth-last)-(child|of-type)(?:\\("+whitespace+"*(even|odd|(([+-]|)(\\d*)n|)"+whitespace+"*(?:([+-]|)"+whitespace+"*(\\d+)|))"+whitespace+"*\\)|)","i"),bool:new RegExp("^(?:"+booleans+")$","i"),needsContext:new RegExp("^"+whitespace+"*[>+~]|:(even|odd|eq|gt|lt|nth|first|last)(?:\\("+whitespace+"*((?:-\\d)?\\d*)"+whitespace+"*\\)|)(?=[^-]|$)","i")},rinputs=/^(?:input|select|textarea|button)$/i,rheader=/^h\d$/i,rnative=/^[^{]+\{\s*\[native \w/,rquickExpr=/^(?:#([\w-]+)|(\w+)|\.([\w-]+))$/,rsibling=/[+~]/,rescape=/'|\\/g,runescape=new RegExp("\\\\([\\da-f]{1,6}"+whitespace+"?|("+whitespace+")|.)","ig"),funescape=function(_,escaped,escapedWhitespace){var high="0x"+escaped-65536;return high!==high||escapedWhitespace?escaped:high<0?String.fromCharCode(high+65536):String.fromCharCode(high>>10|55296,high&1023|56320)},unloadHandler=function(){setDocument()};try{push.apply(arr=slice.call(preferredDoc.childNodes),preferredDoc.childNodes);arr[preferredDoc.childNodes.length].nodeType}catch(e){push={apply:arr.length?function(target,els){push_native.apply(target,slice.call(els))}:function(target,els){var j=target.length,i=0;while(target[j++]=els[i++]){}target.length=j-1}}}function Sizzle(selector,context,results,seed){var match,elem,m,nodeType,i,groups,old,nid,newContext,newSelector;if((context?context.ownerDocument||context:preferredDoc)!==document){setDocument(context)}context=context||document;results=results||[];nodeType=context.nodeType;if(typeof selector!=="string"||!selector||nodeType!==1&&nodeType!==9&&nodeType!==11){return results}if(!seed&&documentIsHTML){if(nodeType!==11&&(match=rquickExpr.exec(selector))){if(m=match[1]){if(nodeType===9){elem=context.getElementById(m);if(elem&&elem.parentNode){if(elem.id===m){results.push(elem);return results}}else{return results}}else{if(context.ownerDocument&&(elem=context.ownerDocument.getElementById(m))&&contains(context,elem)&&elem.id===m){results.push(elem);return results}}}else if(match[2]){push.apply(results,context.getElementsByTagName(selector));return results}else if((m=match[3])&&support.getElementsByClassName){push.apply(results,context.getElementsByClassName(m));return results}}if(support.qsa&&(!rbuggyQSA||!rbuggyQSA.test(selector))){nid=old=expando;newContext=context;newSelector=nodeType!==1&&selector;if(nodeType===1&&context.nodeName.toLowerCase()!=="object"){groups=tokenize(selector);if(old=context.getAttribute("id")){nid=old.replace(rescape,"\\$&")}else{context.setAttribute("id",nid)}nid="[id='"+nid+"'] ";i=groups.length;while(i--){groups[i]=nid+toSelector(groups[i])}newContext=rsibling.test(selector)&&testContext(context.parentNode)||context;newSelector=groups.join(",")}if(newSelector){try{push.apply(results,newContext.querySelectorAll(newSelector));return results}catch(qsaError){}finally{if(!old){context.removeAttribute("id")}}}}}return select(selector.replace(rtrim,"$1"),context,results,seed)}function createCache(){var keys=[];function cache(key,value){if(keys.push(key+" ")>Expr.cacheLength){delete cache[keys.shift()]}return cache[key+" "]=value}return cache}function markFunction(fn){fn[expando]=true;return fn}function assert(fn){var div=document.createElement("div");try{return!!fn(div)}catch(e){return false}finally{if(div.parentNode){div.parentNode.removeChild(div)}div=null}}function addHandle(attrs,handler){var arr=attrs.split("|"),i=attrs.length;while(i--){Expr.attrHandle[arr[i]]=handler}}function siblingCheck(a,b){var cur=b&&a,diff=cur&&a.nodeType===1&&b.nodeType===1&&(~b.sourceIndex||MAX_NEGATIVE)-(~a.sourceIndex||MAX_NEGATIVE);if(diff){return diff}if(cur){while(cur=cur.nextSibling){if(cur===b){return-1}}}return a?1:-1}function createInputPseudo(type){return function(elem){var name=elem.nodeName.toLowerCase();return name==="input"&&elem.type===type}}function createButtonPseudo(type){return function(elem){var name=elem.nodeName.toLowerCase();return(name==="input"||name==="button")&&elem.type===type}}function createPositionalPseudo(fn){return markFunction(function(argument){argument=+argument;return markFunction(function(seed,matches){var j,matchIndexes=fn([],seed.length,argument),i=matchIndexes.length;while(i--){if(seed[j=matchIndexes[i]]){seed[j]=!(matches[j]=seed[j])}}})})}function testContext(context){return context&&typeof context.getElementsByTagName!=="undefined"&&context}support=Sizzle.support={};isXML=Sizzle.isXML=function(elem){var documentElement=elem&&(elem.ownerDocument||elem).documentElement;return documentElement?documentElement.nodeName!=="HTML":false};setDocument=Sizzle.setDocument=function(node){var hasCompare,parent,doc=node?node.ownerDocument||node:preferredDoc;if(doc===document||doc.nodeType!==9||!doc.documentElement){return document}document=doc;docElem=doc.documentElement;parent=doc.defaultView;if(parent&&parent!==parent.top){if(parent.addEventListener){parent.addEventListener("unload",unloadHandler,false)}else if(parent.attachEvent){parent.attachEvent("onunload",unloadHandler)}}documentIsHTML=!isXML(doc);support.attributes=assert(function(div){div.className="i";return!div.getAttribute("className")});support.getElementsByTagName=assert(function(div){div.appendChild(doc.createComment(""));return!div.getElementsByTagName("*").length;
